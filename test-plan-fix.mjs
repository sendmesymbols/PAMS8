import Plan from './MS/Engines/ImportExport/Plan.js';
...
testCases.forEach((testCase) => {
  ...
  try {
    const result = Plan.normalizeDrawEssForLegacyExport(testCase.input);
    ...
    if (testCase.expectedOptionsTypes) {
      ...
      for (const [field, expectedType] of Object.entries(testCase.expectedOptionsTypes)) {
        ...
        if (actualType !== expectedType) {
          console.log(`  ❌ FAIL: Expected OPTIONS.${field} to be type ${expectedType}, got ${actualType} (value: ${JSON.stringify(options[field])})`);
          failedTests++;
          return;
        }
      }
