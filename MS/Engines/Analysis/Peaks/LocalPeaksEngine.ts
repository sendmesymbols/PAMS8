...
export class LocalPeaksEngine {
  ...
  private _showPanel(): void {
    if (!this._panelEl) {
      ...
      this._panelEl.setAttribute('data-engine', 'peaks');
  ...
  private _setStatus(message: string, tone: StatusTone): void {
    ...
    if (status) { status.textContent = message; status.className = `ms-status ${tone === 'running' ? 'running' : tone === 'warn' ? 'warning' : tone === 'pick' ? '' : 'success'}`; }
    const dot = this._el('peaks-status-dot');
    const lbl = this._el('peaks-status-lbl');
    const map: Record<StatusTone, string> = { ready: 'ready', running: 'running', warn: 'warning', pick: '', done: 'ready' };
    const dotClass = map[tone];
    if (dot) { dot.className = `ms-status-dot ${dotClass}`.trim(); }
    if (lbl) lbl.textContent = tone === 'ready' ? 'Ready' : tone === 'running' ? 'Running' : tone === 'warn' ? 'Check' : tone === 'pick' ? 'Pick' : 'Done';
  }
