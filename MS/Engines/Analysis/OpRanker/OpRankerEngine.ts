...
export class OpRankerEngine {
  ...
  initialize(view: MapView | SceneView): void {
    ...
    if (this._controlPanelEl?.classList.contains('ms-visible')) this._bindMapClick();
  }
  ...
  private _ensurePanels(): void {
    if (!this._listPanelEl) {
      ...
      this._listPanelEl.setAttribute('data-engine', 'op-ranker');
      this._listPanelEl.style.cssText = 'top: 60px; left: 14px; width: 480px;';
      this._listPanelEl.innerHTML = this._listPanelHtml();
      document.body.appendChild(this._listPanelEl);
      this._makeDraggable(this._listPanelEl, '.ms-header');
    }
    if (!this._controlPanelEl) {
      ...
      this._controlPanelEl.setAttribute('data-engine', 'op-ranker');
      this._controlPanelEl.style.cssText = 'top: 60px; right: 14px; width: 404px;';
      this._controlPanelEl.innerHTML = this._controlPanelHtml();
      document.body.appendChild(this._controlPanelEl);
      this._bindPanelEvents();
      this._makeDraggable(this._controlPanelEl, '.ms-header');
    }
    if (!this._hintEl) {
  ...
  private _listPanelHtml(): string {
    return `
      <div class="ms-header">
        <div class="ms-header-icon">OP</div>
        <div class="ms-header-title">OP Ranker — Ranked</div>
        <div class="ms-status-lbl" id="oprank-lph-sub">Place OPs on map then run analysis</div>
      </div>
      <div class="buffer-stats" id="oprank-summary-strip">
        <div class="buffer-stat"><div class="buffer-stat-lbl">OPs placed</div><div class="buffer-stat-val" id="oprank-ss-ops">0</div></div>
        <div class="buffer-stat"><div class="buffer-stat-lbl">AO coverage</div><div class="buffer-stat-val" id="oprank-ss-cov">-</div></div>
        <div class="buffer-stat"><div class="buffer-stat-lbl">Gap area</div><div class="buffer-stat-val" id="oprank-ss-gap">-</div></div>
      </div>
      ...
      <div class="ms-body" id="oprank-op-list"></div>
    `;
  }

  private _controlPanelHtml(): string {
    return `
      <div class="ms-header" id="oprank-drag-handle">
        <div class="ms-header-icon">OP</div>
        <div class="ms-header-title">OP Ranker</div>
        <div class="ms-status-dot" id="oprank-status-dot"></div>
        <div class="ms-status-lbl" id="oprank-status">Place OPs</div>
        <button class="ms-header-btn ms-btn-round" id="oprank-help-btn" title="How OP ranking works">?</button>
        <button class="ms-header-btn ms-btn-round" id="oprank-min-btn" title="Minimize">▼</button>
        <button class="ms-header-btn ms-btn-round" id="oprank-close-btn" title="Close (keeps graphics)">✕</button>
      </div>
      <div class="ms-help-popover" id="oprank-help-popover" hidden>
        <div class="ms-help-head">
          <div><div class="ms-help-kicker">Field Guide</div><div class="ms-help-title">Observation Post Ranker</div></div>
          <button id="oprank-help-close" class="ms-help-close">✕</button>
        </div>
        <div class="ms-help-body">
          <div class="ms-help-answers">
            <div class="ms-help-answers-kicker">Answers</div>
            <div class="ms-help-answers-q">Which of these OPs should I actually man?</div>
          </div>
          ...
          <div class="ms-help-block"><h4>Workflow</h4>
            <ol>
              <li><strong>Set target</strong> — click the objective so coverage is scored against it, not the space between your own OPs.</li>
              ...
              <li>Platform, grid and overlay options live in the disclosures; <em>Export Report</em> appears once ranked.</li>
            </ol>
          </div>
          <div class="ms-help-block"><h4>Composite Score (0–100)</h4>
            <dl>
          ...
          <div class="ms-help-block"><h4>AO Centre</h4>
          ...
          <div class="ms-help-block"><h4>Mutual LOS</h4>
      ...
      <div class="ms-body">
        <!-- Default view: set the target, click the map to drop candidate OPs
             (the map itself is the OP-picker, up to 8), choose the AO radius,
             run. Platform, grid, set size and overlays live in the disclosures. -->
        <div class="ms-section-title">Target &amp; area</div>
        <div class="ms-btn-row">
          <button class="ms-btn primary" id="oprank-btn-ao-pick" title="Click, then click the map to set the observation target">📍 Set target on map</button>
          <button class="ms-btn" id="oprank-btn-ao-clear" style="flex:0 0 auto" hidden>Clear target</button>
        </div>
        <div id="oprank-ao-coords" class="oprank-ao-note">No target set — AO will centre on OP centroid</div>
        <div class="ms-grid full">
          <div class="ms-field">
            <label class="ms-label" for="oprank-inp-ao">AO radius — coverage scoring boundary</label>
            <select id="oprank-inp-ao" class="ms-select">
        ...
        <div class="ms-btn-row">
          <button class="ms-btn ms-cta" id="oprank-btn-run" disabled title="Place at least 2 OPs on the map first">Run Analysis ↗</button>
        </div>

        <div class="ms-progress-wrap" id="oprank-prog-wrap" hidden><div class="ms-progress-track"><div class="ms-progress-fill" id="oprank-prog-fill"></div></div><div class="ms-progress-label" id="oprank-prog-label">-</div></div>
        ...
        <div class="ms-btn-row" id="oprank-actions-row" hidden>
          <button class="ms-btn danger" id="oprank-btn-clear">Clear all</button>
          <button class="ms-btn" id="oprank-btn-export" disabled hidden>Export Report</button>
        </div>

        <div class="ms-disclosure" data-open="false">
          <button class="ms-disclosure-head" type="button" id="oprank-adv-toggle" aria-expanded="false" aria-controls="oprank-adv-body">
            <span class="ms-disclosure-chevron" aria-hidden="true">▶</span>
            <span class="ms-disclosure-title">Advanced</span>
            <span class="ms-disclosure-meta">Platform, grid, set size</span>
          </button>
          <div class="ms-disclosure-body" id="oprank-adv-body" hidden>
            <div class="ms-section-title">Observer platform</div>
            ...
            <div class="ms-grid">
              <div class="ms-field"><label class="ms-label" for="oprank-inp-eye">Eye height (m)</label><input id="oprank-inp-eye" class="ms-input" type="number" value="1.8" min="0.5" max="20" step="0.1"/></div>
              <div class="ms-field"><label class="ms-label" for="oprank-inp-range">Max obs range (m)</label><input id="oprank-inp-range" class="ms-input" type="number" value="5000" min="500" max="20000" step="200"/></div>
            </div>

            <div class="ms-section-title">Grid resolution</div>
            <div class="ms-grid full">
              <div class="ms-field">
                <label class="ms-label" for="oprank-inp-cell">Cell size (m)</label>
                <select id="oprank-inp-cell" class="ms-select">
            ...
            <div class="ms-section-title">Optimal OP set</div>
            <div class="ms-grid full">
              <div class="ms-field">
                <label class="ms-label" for="oprank-inp-optimal">How many OPs to recommend</label>
                <select id="oprank-inp-optimal" class="ms-select">
        ...
        <div class="ms-disclosure" data-open="false">
          <button class="ms-disclosure-head" type="button" id="oprank-ov-toggle" aria-expanded="false" aria-controls="oprank-ov-body">
            <span class="ms-disclosure-chevron" aria-hidden="true">▶</span>
            <span class="ms-disclosure-title">Overlays</span>
            <span class="ms-disclosure-meta">Coverage, heatmap, gaps, mutual LOS</span>
          </button>
          <div class="ms-disclosure-body" id="oprank-ov-body" hidden>
            ...
            <div class="ms-slider-row">
              ...
              <div class="ms-slider-value" id="oprank-opa-v">0.70</div>
            </div>
            <div class="ms-toggle-row">
              <label for="oprank-opt-individual">Show individual viewsheds</label>
              <input id="oprank-opt-individual" type="checkbox" class="ms-input"/>
            </div>
            <div class="ms-toggle-row">
              <label for="oprank-opt-overlap">Show OP-OP overlap extent</label>
              <input id="oprank-opt-overlap" type="checkbox" class="ms-input" checked/>
            </div>
          </div>
        </div>
      </div>
  ...
  private _bindPanelEvents(): void {
    ...
    p.querySelector('#oprank-btn-ao-pick')?.addEventListener('click', () => {
      ...
      if (!btn) return;
      if (this._aoPickMode) {
        btn.textContent = '\u2715 Cancel target pick';
  ...
  private _bindMapClick(): void {
    ...
    this._clickHandle = this._view.on('click', async (event: any) => {
      ...
      if (this._aoPickMode) {
        ...
        if (pickBtn) { pickBtn.textContent = '📍 Set target on map'; pickBtn.classList.remove('ms-armed'); }
  ...
  private _clearAll(): void {
    ...
    const exp = this._button('oprank-btn-export');
    if (exp) { exp.hidden = true; exp.disabled = true; }
    this._cachedCoverage = null;
    this._opLayer.removeAll();
    this._rangeLayer.removeAll();
    this._aoLayer.removeAll();
  ...
  private async _runAnalysis(): Promise<void> {
    ...
    try {
      ...
      const exportBtn = this._button('oprank-btn-export');
      if (exportBtn) { exportBtn.disabled = false; exportBtn.hidden = false; }
  ...
  private _setRunBusy(busy: boolean): void {
    const btn = this._button('oprank-btn-run');
    if (!btn) return;
    btn.disabled = busy || this._ops.length < 2;
  ...
  private _setProgressVisible(visible: boolean): void {
    const el = this._el('oprank-prog-wrap');
  ...
  private _hidePanels(): void {
    this._listPanelEl?.classList.remove('ms-visible');
  ...
  private _syncSummary(): void {
    ...
    if (run) {
      run.disabled = this._ops.length < 2 || this._running;
      run.title = this._ops.length < 2 ? 'Place at least 2 OPs on the map first' : 'Rank the placed OPs';
    }
    // Clear all / Export only apply once something exists to clear or export.
    const actions = this._el('oprank-actions-row');
    if (actions) actions.hidden = this._ops.length === 0 && !this._aoCenterPt;
  }
  ...
  private _setStatus(state: 'placing' | 'computing' | 'done' | 'error', text: string): void {
    ...
    const dot = this._el('oprank-status-dot');
    if (dot) {
      const color = state === 'computing' ? 'var(--ms-accent)'
        : state === 'done' ? 'var(--ms-success)'
        : state === 'error' ? 'var(--ms-danger, #dc5050)'
        : '#888';
      dot.style.background = color;
      dot.style.boxShadow = state === 'placing' ? 'none' : `0 0 6px ${color}`;
    }
  }
