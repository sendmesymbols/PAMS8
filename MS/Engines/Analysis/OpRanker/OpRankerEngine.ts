  ...
  private _controlPanelHtml(): string {
    return `
      ...
      <div class="oprank-help-popover" id="oprank-help-popover" hidden>
        <div class="oprank-help-head"><div><div class="oprank-help-kicker">Field Guide</div><div class="oprank-help-title">Observation Post Ranker</div></div><button id="oprank-help-close" class="oprank-help-close">x</button></div>
        <div class="oprank-help-body">
          ...
          <div class="oprank-help-block"><h4>What it scores</h4><dl><dt>Visibility</dt><dd>Total AO cells the OP can see (viewshed extent under the observer eye height).</dd><dt>Unique cover</dt><dd>Cells only this OP can see — measures how irreplaceable the post is.</dd><dt>Masking</dt><dd>Terrain blocking between the OP and the AO; high masking = low coverage.</dd><dt>Cover</dt><dd>Local concealment for the observer (ridge-line / reverse-slope advantage).</dd><dt>Range coverage</dt><dd>Fraction of the configured max-observation range that yields usable LOS.</dd></dl></div>
          <div class="oprank-help-block"><h4>Reading the ranked list</h4><p>Cards are sorted by composite score (higher is better). The green left border marks OPs included in the recommended optimal set. The badge in the top-right is the composite score; bars below show unique vs total coverage. The optimal-set strip summarises the greedy max-coverage choice and the AO percentage it covers together.</p></div>
          <div class="oprank-help-block"><h4>Weights</h4><p>The composite score weights unique coverage highest, then total viewshed, then masking penalty. Adjusting AO radius reweights all scores because it changes the denominator each OP is judged against; smaller AO favours close-in OPs, larger AO favours OPs with long sight-lines.</p></div>
  ...
  private _injectStyles(): void {
    ...
    style.textContent = `
      #oprank-list-panel,#oprank-ctrl-panel{
        ...
        --ms-accent: #378ADD;
        --ms-accent-2: #1D9E75;
        position:fixed;z-index:1100;background:var(--ms-bg);border:1px solid var(--ms-border);border-radius:var(--ms-radius,9px);color:var(--ms-text);font-family:var(--ms-font,'SF Pro Display','Segoe UI',system-ui,sans-serif);font-size:var(--ms-fs);box-shadow:var(--ms-shadow,0 8px 36px rgba(0,0,0,.55),inset 0 0 0 1px rgba(255,255,255,.04));user-select:none;overflow:hidden}
      #oprank-list-panel{top:60px;left:14px;width:380px;max-height:calc(100vh - 88px);display:none;flex-direction:column}
      #oprank-ctrl-panel{top:60px;right:14px;width:380px;max-height:calc(100vh - 88px);overflow-y:auto;display:none}
      ...
      .oprank-ph-title{flex:1}.oprank-ph-tools{display:flex;gap:3px}.oprank-help-btn,.oprank-min-btn,.oprank-close-btn,.oprank-help-close{background:var(--ms-bg-input);border:1px solid var(--ms-border);color:var(--ms-text-dim);border-radius:3px;font-family:inherit;font-size:var(--ms-fs);cursor:pointer}
      .oprank-help-btn{width:19px;height:19px;border-radius:50%;color:var(--ms-accent-2);font-weight:700}.oprank-help-btn:hover,.oprank-min-btn:hover,.oprank-close-btn:hover,.oprank-help-close:hover{color:var(--ms-text)}
      .oprank-ph-status{font-size:var(--ms-fs-xs);letter-spacing:.07em;text-transform:uppercase;color:var(--ms-text-label)}.oprank-ph-status.placing{color:var(--ms-accent)}.oprank-ph-status.computing{color:#EF9F27}.oprank-ph-status.done{color:var(--ms-accent-2)}.oprank-ph-status.error{color:#DC3C30}
      #oprank-summary-strip{display:grid;grid-template-columns:repeat(3,1fr);border-bottom:1px solid var(--ms-divider);flex-shrink:0}.oprank-ss-cell{padding:8px 11px;display:flex;flex-direction:column;gap:2px;border-right:1px solid var(--ms-divider)}.oprank-ss-cell:last-child{border-right:none}.oprank-ss-l{font-size:var(--ms-fs-xs);letter-spacing:.08em;text-transform:uppercase;color:var(--ms-text-label);font-weight:600}.oprank-ss-v{font-size:var(--ms-fs-sm);font-weight:700;color:var(--ms-accent)}
      #oprank-op-list{overflow-y:auto;flex:1;padding:7px}.oprank-op-card{background:var(--ms-bg-input);border:1px solid var(--ms-divider);border-radius:5px;padding:9px 11px;margin-bottom:6px;cursor:pointer;transition:all .12s;position:relative}.oprank-op-card:hover{background:var(--ms-bg-header);border-color:var(--ms-accent)}.oprank-op-card.selected{border-color:var(--ms-accent);background:var(--ms-bg-header)}.oprank-op-card.in-optimal{border-left:3px solid var(--ms-accent-2)}.oprank-op-top{display:flex;align-items:center;gap:8px;margin-bottom:6px}.oprank-op-num{width:26px;height:26px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:var(--ms-fs);font-weight:700;flex-shrink:0;background:rgba(55,138,221,.15);color:var(--ms-accent);border:1px solid var(--ms-border)}.oprank-op-num.optimal{background:rgba(29,158,117,.15);color:var(--ms-accent-2);border-color:rgba(29,158,117,.5)}
      .oprank-op-name{font-size:var(--ms-fs);font-weight:600;color:var(--ms-text);flex:1}.oprank-op-coords{font-size:var(--ms-fs-xs);color:var(--ms-text-label);letter-spacing:.04em;margin-top:1px}.oprank-op-badge{font-size:var(--ms-fs-sm);font-weight:700;position:absolute;top:9px;right:10px}.oprank-colour-dot{width:9px;height:9px;border-radius:50%;display:inline-block;flex-shrink:0}
      .oprank-op-bars{display:grid;grid-template-columns:1fr 1fr;gap:6px 10px;margin-bottom:7px}.oprank-ob{display:flex;flex-direction:column;gap:2px}.oprank-ob-lbl{font-size:var(--ms-fs-xs);letter-spacing:.07em;text-transform:uppercase;color:var(--ms-text-label);font-weight:600}.oprank-ob-track{height:4px;background:rgba(255,255,255,.06);border-radius:2px}.oprank-ob-fill{height:100%;border-radius:2px}.oprank-ob-val{font-size:var(--ms-fs-xs);color:var(--ms-text-dim)}.oprank-op-note{font-size:var(--ms-fs-xs);color:var(--ms-text-dim);line-height:1.5;border-top:1px solid var(--ms-divider);padding-top:6px}.oprank-op-actions{display:flex;gap:5px;margin-top:7px}.oprank-op-action-btn{font-size:var(--ms-fs-xs);letter-spacing:.05em;text-transform:uppercase;padding:4px 8px;border-radius:3px;border:1px solid var(--ms-border);background:transparent;color:var(--ms-text-dim);cursor:pointer;font-family:inherit;font-weight:600}.oprank-op-action-btn:hover{background:var(--ms-bg-header);color:var(--ms-text)}.oprank-op-action-btn.remove{border-color:rgba(220,60,48,.4);color:rgba(220,80,72,.85)}
      #oprank-list-empty{padding:22px 14px;font-size:var(--ms-fs);color:var(--ms-text-label);text-align:center;line-height:1.8}
      .oprank-ps{font-size:var(--ms-fs);letter-spacing:.12em;text-transform:uppercase;color:var(--ms-accent);font-weight:700;padding:11px 14px 6px}.oprank-pg{display:grid;grid-template-columns:1fr 1fr;gap:9px 10px;padding:0 12px 9px}.oprank-pf{display:flex;flex-direction:column;gap:3px}.oprank-pf.full{grid-column:1/-1}.oprank-pl{font-size:var(--ms-fs-xs);letter-spacing:.07em;text-transform:uppercase;color:var(--ms-text-label);font-weight:600}
      #oprank-ctrl-panel input,#oprank-ctrl-panel select{background:var(--ms-bg-input);border:1px solid var(--ms-border);border-radius:4px;color:var(--ms-text);font-family:inherit;font-size:var(--ms-fs);padding:7px 9px;width:100%;outline:none;transition:border-color .15s;box-sizing:border-box}#oprank-ctrl-panel input:focus,#oprank-ctrl-panel select:focus{border-color:var(--ms-accent)}#oprank-ctrl-panel select option{background:var(--ms-bg)}
      .oprank-psr{display:flex;align-items:center;gap:8px;padding:0 12px 8px}.oprank-psr-l{font-size:var(--ms-fs-xs);letter-spacing:.07em;text-transform:uppercase;color:var(--ms-text-label);flex:1.8;font-weight:600}.oprank-psr input[type=range]{flex:2;accent-color:var(--ms-accent);cursor:pointer}.oprank-psr-v{font-size:var(--ms-fs);color:var(--ms-accent);min-width:42px;text-align:right;font-weight:600}.oprank-pdiv{height:1px;background:var(--ms-divider);margin:6px 0}.oprank-ptr{display:flex;align-items:center;justify-content:space-between;padding:6px 14px}.oprank-ptr label{font-size:var(--ms-fs-xs);letter-spacing:.07em;text-transform:uppercase;color:var(--ms-text-label);cursor:pointer;font-weight:600}.oprank-ptr input[type=checkbox]{accent-color:var(--ms-accent);width:15px;height:15px;cursor:pointer}
      .oprank-ov-row{display:flex;gap:6px;padding:0 12px 9px}.oprank-ov-btn{flex:1;padding:7px 4px;font-size:var(--ms-fs-xs);letter-spacing:.05em;text-transform:uppercase;cursor:pointer;border-radius:4px;border:1px solid var(--ms-border);background:var(--ms-bg-input);color:var(--ms-text-dim);font-family:inherit;transition:all .13s;font-weight:600}.oprank-ov-btn:hover{background:var(--ms-bg-header);color:var(--ms-text)}.oprank-ov-btn.on{border-color:var(--ms-accent);color:var(--ms-accent);background:rgba(55,138,221,.12)}.oprank-ov-btn.gap-on{border-color:rgba(220,60,48,.55);color:#DC3C30;background:rgba(220,60,48,.12)}
      #oprank-prog-wrap{padding:0 12px 9px}#oprank-prog-track{height:6px;background:var(--ms-bg-input);border:1px solid var(--ms-divider);border-radius:3px;overflow:hidden}#oprank-prog-fill{height:100%;background:linear-gradient(90deg,var(--ms-accent-2),var(--ms-accent));border-radius:2px;width:0%;transition:width .16s}#oprank-prog-label{font-size:var(--ms-fs-xs);color:var(--ms-text-label);letter-spacing:.05em;margin-top:5px}
      .oprank-optimal-strip{margin:0 12px 9px;padding:9px 11px;background:rgba(29,158,117,.07);border:1px solid rgba(29,158,117,.28);border-radius:4px}.oprank-opt-label{font-size:var(--ms-fs-xs);letter-spacing:.08em;text-transform:uppercase;color:var(--ms-accent-2);margin-bottom:5px;font-weight:700}.oprank-opt-body{font-size:var(--ms-fs);color:var(--ms-text-dim);line-height:1.6}
      .oprank-pb-row{display:flex;gap:7px;padding:9px 12px 11px}.oprank-pb{flex:1;padding:8px 5px;font-family:inherit;font-size:var(--ms-fs-xs);letter-spacing:.06em;text-transform:uppercase;cursor:pointer;border-radius:4px;border:1px solid var(--ms-border);background:var(--ms-bg-input);color:var(--ms-text-dim);font-weight:600;transition:all .14s}.oprank-pb:hover:not(:disabled){background:var(--ms-bg-header);color:var(--ms-text)}.oprank-pb.primary{background:rgba(55,138,221,.14);border-color:var(--ms-accent);color:var(--ms-accent);font-weight:700}.oprank-pb:disabled{opacity:.35;cursor:not-allowed}
      #oprank-hint{position:fixed;bottom:55px;left:50%;transform:translateX(-50%);background:rgba(26,32,48,.78);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid rgba(55,138,221,.45);box-shadow:0 4px 16px rgba(0,0,0,.35);color:#7FB8EA;font-family:var(--ms-font,'SF Pro Display','Segoe UI',system-ui,sans-serif);font-size:var(--ms-fs);letter-spacing:.08em;padding:9px 22px;border-radius:6px;pointer-events:none;z-index:1100;text-transform:uppercase;display:none;font-weight:600}
      .oprank-help-popover{position:absolute;top:39px;left:8px;right:8px;z-index:1120;max-height:min(520px,calc(100vh - 132px));overflow-y:auto;background:var(--ms-bg);border:1px solid var(--ms-border);border-radius:var(--ms-radius,9px);box-shadow:var(--ms-shadow,0 8px 36px rgba(0,0,0,.55));color:var(--ms-text)}.oprank-help-popover[hidden]{display:none}.oprank-help-head{display:flex;justify-content:space-between;gap:10px;padding:11px 12px 9px;border-bottom:1px solid var(--ms-divider);background:var(--ms-bg-header)}.oprank-help-kicker{font-size:var(--ms-fs-xs);color:var(--ms-text-label);letter-spacing:.09em;text-transform:uppercase;font-weight:600}.oprank-help-title{margin-top:2px;font-size:var(--ms-fs-sm);color:var(--ms-accent);font-weight:700}.oprank-help-body{padding:11px 12px 13px;font-size:var(--ms-fs);line-height:1.5;color:var(--ms-text-dim);user-select:text}.oprank-help-body p{margin:0 0 9px}.oprank-help-block{margin-top:11px}.oprank-help-block h4{margin:0 0 6px;font-size:var(--ms-fs);letter-spacing:.08em;text-transform:uppercase;color:var(--ms-text);font-weight:700}.oprank-help-block ol{margin:0;padding-left:18px}.oprank-help-block li{margin:4px 0}.oprank-help-block dl{display:grid;grid-template-columns:90px minmax(0,1fr);gap:6px 10px;margin:0}.oprank-help-block dt{color:var(--ms-accent);font-weight:700}.oprank-help-block dd{margin:0}
      @media (max-width:720px){#oprank-list-panel{left:10px;top:62px;width:calc(100vw - 20px);max-height:34vh}#oprank-ctrl-panel{left:10px;right:auto;top:calc(34vh + 72px);width:calc(100vw - 20px);max-height:calc(66vh - 88px)}.oprank-pg{grid-template-columns:1fr}}
    `;
