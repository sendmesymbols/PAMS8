  ...
  try {
    ...
    if (testCase.expectedFields) {
      for (const [field, expectedValue] of Object.entries(testCase.expectedFields)) {
        if ((result as any)[field] !== expectedValue) {
          console.log(`  ❌ FAIL: Expected ${field} to be ${expectedValue}, got ${(result as any)[field]}`);
          failedTests++;
          return;
        }
      }
    ...
    if (testCase.expectedAmplifierFields) {
      const amplifier = (result as any).AMPLIFIER;
