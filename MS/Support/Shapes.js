...
define(["esri/geometry/Polyline", "esri/geometry/Point", "MilSymbologyExt/GeoTools",
    "esri/geometry/Circle", "MilSymbologyComponents/Echelons"
    ],
    function (Polyline, Point, GeoTools,
        Circle, Echelons) {
            return {
                ...
                createBufferCircle: function (pt, radius) {
                    var circleGeometry = new Circle({
                        center: pt,
                        radius: radius
                    });

                    var circleGeomJson = circleGeometry.toJson();
                    var circleRingPts = circleGeomJson.rings[0];

                    var pts = [];
                    for (var i = 0; i < circleRingPts.length; i++) {
                        var circlePt = circleRingPts[i];
                        pts.push(new Point(circlePt[0], circlePt[1], pt.spatialReference));
                    }
                ...
                createISR: function (dx, dy, dr, sp) {
                    var pts = [];
                    pts = this.createI(dx - (dr * 1.3), dy, dr, sp);
                    return [pts, this.createS(dx, dy, dr, sp), this.createR(dx + (dr * 1.2), dy, dr, sp)];
                },
                createPlus: function (dx, dy, dr, sp) {
                    var pts = [];
                    //For Horizontal Arm
                    pts.push(new Point(dx, dy, sp));
                    pts.push(new Point(dx - (dr * 0.9), dy, sp));
                    pts.push(new Point(dx, dy, sp));
                    pts.push(new Point(dx + (dr * 0.9), dy, sp));

                    //For Vertical Arm, we used I alphabet
                    return [pts, this.createI(dx, dy, dr, sp)];
                },
                ...
                createCurvedFins: function (pt, radius, pt2) {
                    var dx = pt.x;
                    var dy = pt.y;
                    var xPaths = [], curvedXPaths = [];

                    xPaths = this.createDiamond(dx, dy, radius, pt.spatialReference);

                    return [xPaths, curvedXPaths];
                },
                findPerpendicularLinePts: function (p1, p2) {
                    ...
                    var b = p2.y - (mInverse) * p2.x;

                    var x3 = p2.x + delta;
                    var y3 = (mInverse) * x3 + b;

                    var x4 = p2.x - delta;
                    var y4 = (mInverse) * x4 + b;
                ...
                createDiamond: function (dx, dy, dr, sp) {
                    var pts = [];
                    
                    //Diamond Bottom Point
                    pts.push(new Point(dx + (dr / 2), dy - dr, sp));                    
                    //Diamond Left Center Point
                    pts.push(new Point(dx, dy, sp));                    
                    //Diamond Top Point
                    pts.push(new Point(dx + (dr / 2), dy + dr, sp));                    
                    ...
                    //Diamond Right Center Point
                    pts.push(new Point(dx2, dy, sp));
                    //Diamond Bottom Point Again
                    pts.push(new Point(dx + (dr / 2), dy - dr, sp));
                    
                    return pts;
                },
                createCurvedLine: function (segmentPaths) {
                    var curvedPath = [];
                    var startPoint = segmentPaths[0];
                    var endPoint = segmentPaths[segmentPaths.length - 1];
                    var midPoint = [(startPoint[0] + endPoint[0])/2, (startPoint[1] + endPoint[1])/2];

                    curvedPath.push(startPoint);
                    for (var j=1; j<=5; j++){
                        var t = j/5;
                        var x = (1 - t) * (1 - t) * startPoint[0] + 2 * (1 - t) * t * midPoint[0] + t * t * endPoint[0];
                        var y = (1 - t) * (1 - t) * startPoint[1] + 2 * (1 - t) * t * midPoint[1] + t * t * endPoint[1];
                        curvedPath.push([x,y]);
