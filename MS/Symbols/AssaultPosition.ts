...
export interface AssaultPositionOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polygon | number[][][];
  DRAW_TYPE?: number;
...
/**
 * AssaultPosition class for Assault Position symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes ASLT inner text markers
 */
export class AssaultPosition {
  ...
  // Symbol properties
  public declaredClass = 'MilitarySymbology.Symbols.AssaultPosition';
  private SID: string = '151500';
  private symName: string = 'Assault Position';
  ...
  constructor(view: MapView | SceneView, isLine: boolean = false) {
    ...
    this.events = new SymbolEvents(view, 'AssaultPosition');
   ...
   */
  public init(options: AssaultPositionOptions, marker: SimpleLineSymbol): void {
  ...
  /**
   * Create ASLT inner text markers as rings in the same polygon geometry.
   */
  private createInnerText(
    ...
    try {
      ...
      const rings = this.createASLTRings(midPt, cLenLimit);
  ...
  private createASLTRings(midPt: Point, size: number): number[][][] {
    const letterSize = size * 0.72;
    const step = size * 1.05;
    const spatialReference = midPt.spatialReference;
    const strokes = [
      ...Shapes.createAStrokes(
        midPt.x - step * 1.5,
        midPt.y,
        letterSize,
        spatialReference,
      ),
      ...Shapes.createSStrokes(
        midPt.x - step * 0.5,
        midPt.y,
        letterSize,
        spatialReference,
      ),
      ...Shapes.createLStrokes(
        midPt.x + step * 0.5,
        midPt.y,
        letterSize,
        spatialReference,
      ),
      ...Shapes.createTStrokes(
        midPt.x + step * 1.5,
        midPt.y,
        letterSize,
        spatialReference,
...
export default AssaultPosition;
