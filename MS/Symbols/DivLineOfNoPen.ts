...
class Shapes {
  static createDLNP(x: number, y: number, size: number, spatialRef: any): number[][][] {
    // Create DLNP (Division Line of No Penetration) shape
    const paths: number[][][] = [];
    
    // Create "DLNP" text-like shapes
    // D shape
    paths.push([
      [x - size, y - size],
      [x - size/2, y - size],
...
interface DivLineOfNoPenOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polyline;
  opacity?: number;
}

export default class DivLineOfNoPen extends Evented {
  ...
  public init(options: DivLineOfNoPenOptions, marker: any): void {
    ...
    if (this._lineSym && this._lineSym.color) {
  ...
  private createSymbol(drawEssentials: DrawEssentials): Polyline {
    try {
      ...
      const pt1 = { x: -1 * len * Math.cos(k) + p1.x, y: -1 * len * Math.sin(k) + p1.y };

      const paths1 = Shapes.createDLNP(pt1.x, pt1.y, len / 2, this.view.spatialReference);
      for (let i = 0; i < paths1.length; i++) {
        result.addPath(paths1[i]);
      }

      // Write DLNP at end
      const pt2 = { x: len * Math.cos(k) + p2.x, y: len * Math.sin(k) + p2.y };

      const paths2 = Shapes.createDLNP(pt2.x + (len / 0.5), pt2.y, len / 2, this.view.spatialReference);
