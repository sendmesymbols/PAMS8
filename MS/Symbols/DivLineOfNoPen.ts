...
export interface DivLineOfNoPenOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polyline;
  DRAW_TYPE?: number;
  [key: string]: any;
...
/**
 * DivLineOfNoPen class for drawing Phase Line symbols on MapView or SceneView
 * Creates line symbols with "LNP" text markers at both ends
 */
export class DivLineOfNoPen {
  ...
  // Symbol properties
  public declaredClass: string = "MilitarySymbology.Symbols.DivLineOfNoPen";
  public SID: string = "141900";
  public symName: string = "Div Line of No Pen";
  ...
  constructor(view: MapView | SceneView, isLine: boolean = false) {
    ...
    this.events = new SymbolEvents(view, "DivLineOfNoPen");
   ...
   */
  public init(options: DivLineOfNoPenOptions, marker: SimpleLineSymbol | SimpleFillSymbol): void {
  ...
  private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
    try {
      ...
      // Add DLNP text markers at both ends
      this.addDLNPMarkers(result, p1, p2);
  ...
  /**
   * Add DLNP markers at both ends of the line
   */
  private addDLNPMarkers(result: Polyline, p1: Point, p2: Point): void {
    try {
      const len = GeoTools._2PtLen(p1, p2) / 20;
      const k = GeoTools.angleInRadians(p1, p2);

      // DLNP marker at start point
      const pt1 = {
        x: -1 * len * Math.cos(k) + p1.x,
        y: -1 * len * Math.sin(k) + p1.y
      };

      const dlnpPaths1 = Shapes.createDLNP(pt1.x, pt1.y, len / 2, this.view.spatialReference);
      for (let i = 0; i < dlnpPaths1.length; i++) {
        result.addPath(dlnpPaths1[i]);
      }

      // DLNP marker at end point
      const pt2 = {
        x: len * Math.cos(k) + p2.x,
        y: len * Math.sin(k) + p2.y
      };

      const dlnpPaths2 = Shapes.createDLNP(pt2.x + (len / 0.5), pt2.y, len / 2, this.view.spatialReference);
      for (let i = 0; i < dlnpPaths2.length; i++) {
        result.addPath(dlnpPaths2[i]);
      }

    } catch (e) {
      console.log('Error adding DLNP markers');
    }
  }
