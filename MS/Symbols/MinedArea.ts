...
export interface MinedAreaOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * MinedArea class for the MIL-STD-2525D Mined Area obstacle symbol (25270800).
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3).
 * Renders as an obstacle (green) closed area with the letter "M" at the four
 * cardinal edge-midpoints (top/bottom/left/right), drawn as polygon rings so the
 * letters share the obstacle line symbol (no extra graphics). Copied from
 * NamedAreaOfInterest, with the centred "NAI" inner text replaced by the four "M"s.
 */
export class MinedArea {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.MinedArea";
    private SID: string = "270800";
    private symName: string = "Mined Area";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "MinedArea");
    ...
    /**
     * Initialize the mined area drawing
     */
    public init(options: MinedAreaOptions, marker: SimpleLineSymbol): void {
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polygon | null {
        try {
            ...
            return result ? this.createMineMarkers(result as Polygon) : result;
