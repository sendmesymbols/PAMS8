...
export interface HighDensityAirspaceControlZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * HighDensityAirspaceControlZone class for Target High-Density Airspace Control Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class HighDensityAirspaceControlZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.HighDensityAirspaceControlZone";
    private SID: string = "170900";
    private symName: string = "High-Density Airspace Control Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "HighDensityAirspaceControlZone");
     ...
     */
    public init(options: HighDensityAirspaceControlZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createHIDACZRings
                ? (Shapes as any).createHIDACZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createHIDACZ from the source JS
                const naiTxt = (Shapes as any).createHIDACZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default HighDensityAirspaceControlZone;