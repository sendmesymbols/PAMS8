...
export class DropZone {
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "DropZone");
     ...
     */
    public init(options: DropZoneOptions, marker: SimpleLineSymbol): void {
        this._lineSym = marker.clone();
        this._drawType = Number(options.DRAW_TYPE) || 1;
            ...
            if (options.GEOM && this.tempGraphic) {
                try {
                    this.tempGraphic.geometry = options.GEOM instanceof Polygon
                        ? options.GEOM.clone()
                        : new Polygon({
                            rings: (options.GEOM as any).rings ?? options.GEOM,
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polygon | null {
        try {
            ...
            switch (drawType) {
                case 1: {
                    const smoothGeometry = Shapes.createSymbolByBCurve(pts, firstPoint, lastPoint, drawEssentials, this.view.spatialReference);
                    result = smoothGeometry.type === "polygon" ? smoothGeometry as Polygon : null;
                    break;
                }
                case 2:
    ...
    /**
     * Create inner "DZ" text marker for Drop Zone.
     * Strokes are added as rings to the same polygon so the text is part of
     * the polygon geometry (no extra graphics) and inherits the outline color.
     */
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createDZRings
                ? (Shapes as any).createDZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                for (let r = 0; r < rings.length; r++) {
                    ...
                    if (ring && ring.length >= 4) {
                        ...
                        // Zero-area "there and back" ring → single clean line stroke
