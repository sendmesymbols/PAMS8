...
export interface UXOAreaOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * UXOArea class for the MIL-STD-2525D Unexploded Explosive Ordnance Area
 * symbol (25271000).
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3).
 * Renders as a closed area with "UXO" at the top and bottom edge-midpoints.
 * Labels are polygon rings, keeping the complete symbol in one geometry.
 */
export class UXOArea {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.UXOArea";
    private SID: string = "271000";
    private symName: string = "Unexploded Explosive Ordnance (UXO) Area";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "UXOArea");
    ...
    /**
     * Initialize the UXO area drawing
     */
    public init(options: UXOAreaOptions, marker: SimpleLineSymbol): void {
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polygon | null {
        try {
            ...
            return result ? this.createUXOMarkers(result as Polygon) : result;
    ...
    /**
     * Draw "UXO" at the top and bottom edge-midpoints. Each letter stroke is
     * embedded as a polygon ring so the complete symbol remains one geometry.
     */
    private createUXOMarkers(result: Polygon): Polygon {
        try {
            ...
            const dr = Math.min(extent.width, extent.height) / 14;
            if (!dr || dr <= 0) {
                return result;
            }

            const cx = (extent.xmin + extent.xmax) / 2;
            // Two opposing edge-midpoints: top and bottom.
            ...
            for (let p = 0; p < positions.length; p++) {
                const dx = positions[p][0];
                const dy = positions[p][1];
                const xStrokes = [
                    [
                        new Point({x: dx - dr * 0.5, y: dy - dr, spatialReference: sp}),
                        new Point({x: dx + dr * 0.5, y: dy + dr, spatialReference: sp})
                    ],
                    [
                        new Point({x: dx + dr * 0.5, y: dy - dr, spatialReference: sp}),
                ...
                const strokes = [
                    ...(Shapes as any).createUStrokes(dx - dr * 1.7, dy, dr, sp),
                    ...xStrokes,
                    ...(Shapes as any).createOStrokes(dx + dr * 1.7, dy, dr, sp)
                ];
            ...
            console.log('Cannot create UXO Area markers');
            return result;
        }
    }
