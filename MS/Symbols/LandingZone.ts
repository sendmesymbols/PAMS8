...
export interface LandingZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * LandingZone class for Landing Zone symbol
...
export class LandingZone {
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "LandingZone");
     ...
     */
    public init(options: LandingZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createLZRings
                ? (Shapes as any).createLZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createLZ from the source JS
                const lzTxt = (Shapes as any).createLZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                if (lzTxt && Array.isArray(lzTxt)) {
                    for (let j = 0; j <= lzTxt.length - 1; j++) {
                        if (lzTxt[j]) {
                            result.addRing(lzTxt[j]);
                        }
                    }
                }
            }
