...
export class FriendlyDirOfMainAttk {
  ...
  constructor(view: MapView | SceneView, isLine: boolean = false) {
    ...
    this.events = new SymbolEvents(view, 'FriendlyDirOfMainAttk');
   ...
   */
  public init(
    options: FriendlyDirOfMainAttkOptions,
  ...
  private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
    try {
      ...
      if (pts.length >= 2) {
        ...
        const totalLength = GeoTools._2PtLen(pts[0], lastPoint);
        const outerLength = GeoTools.ArrowFlanksLen(totalLength, totalLength);
        const angle = GeoTools.angleInRadians(secondLastPoint, lastPoint);
        const innerTip = new Point({
          x: lastPoint.x - outerLength * 0.22 * Math.cos(angle),
          y: lastPoint.y - outerLength * 0.22 * Math.sin(angle),
          spatialReference: this.view.spatialReference,
        });
        const outerHead = Shapes.createSimpleArrowHead(
          lastPoint,
          secondLastPoint,
          outerLength,
        );
