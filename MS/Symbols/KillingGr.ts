...
class Shapes {
  static createKG(x: number, y: number, size: number, spatialRef: any): number[][][] {
    // Create KG (Killing Gr) shapes
    const shapes: number[][][] = [];
...
interface KillingGrOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polygon;
  DRAW_TYPE?: number;
}

export default class KillingGr extends Evented {
  public declaredClass: string = "MilitarySymbology.Symbols.KillingGr";
  ...
  public init(options: KillingGrOptions, marker: any): void {
  ...
  private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
    try {
      ...
      if (extent && extent.center) {
        ...
        const kgShapes = Shapes.createKG(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
        for (let j = 0; j <= kgShapes.length - 1; j++) {
          result.addRing(kgShapes[j]);
        }
      }
      return result;
    } catch (e) {
