...
export interface LowLevelTransitRouteOptions {
    CTRL_PTS?: Point[];
...
/**
 * LowLevelTransitRoute class for drawing LowLevelTransitRoute tactical symbols
 * Uses baseline + control points
 * Returns Polyline geometry despite being classified as an Area symbol
 */
export class LowLevelTransitRoute {
    ...
    // Symbol properties
    public SID = "170200";
    public symName = "Low Level Transit Route";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "LowLevelTransitRoute");
    ...
    /**
     * Initialize the LowLevelTransitRoute drawing
     */
    public init(options: LowLevelTransitRouteOptions, marker: SimpleLineSymbol): void {
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
        try {
            ...
            // Centreline track through the baseline midpoint and each control point.
            // Corridor edges are built per-leg afterwards (perpendicular to each leg)
            // so they cross cleanly at the ACP circles instead of shearing at turns.
            ...
            // Add corridor side paths — one straight pair per leg, each offset by the
            // half-width perpendicular to that leg's own direction. Edges of adjacent
            // legs cross at the shared control point (under the ACP circle) instead of
            // shearing past the turn.
            const halfWidth = partialLen;
            for (let i = 0; i < middleArray.length - 1; i++) {
                const a = middleArray[i];
                const b = middleArray[i + 1];
                const dx = b.x - a.x;
                const dy = b.y - a.y;
                ...
                result.addPath([[a.x + nx, a.y + ny], [b.x + nx, b.y + ny]]);
                result.addPath([[a.x - nx, a.y - ny], [b.x - nx, b.y - ny]]);
            }
            ...
            if (values && values.midPoints && Array.isArray(values.midPoints)) {
                for (let i = 0; i < values.midPoints.length; i++) {
                    ...
                    const lltrStrokes = (Shapes as any).createLLTR
                        ? (Shapes as any).createLLTR(values.midPoints[i].midPt.x, values.midPoints[i].midPt.y, cLenLimit, values.midPoints[i].midPt.spatialReference)
                        : [];
                    if (Array.isArray(lltrStrokes)) {
                        for (let j = 0; j <= lltrStrokes.length - 1; j++) {
                            const seg = lltrStrokes[j] as Point[];
                            if (seg && seg.length) {
                                result.addPath(seg.map(p => [p.x, p.y]));
                            }
                        }
...
export default LowLevelTransitRoute;
