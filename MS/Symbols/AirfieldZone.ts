...
export interface AirfieldZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * AirfieldZone class for Airfield Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner horizontal X marker
 */
export class AirfieldZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.AirfieldZone";
    private SID: string = "25120400";
    private symName: string = "Airfield Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "AirfieldZone");
     ...
     */
    public init(options: AirfieldZoneOptions, marker: SimpleLineSymbol): void {
    ...
    /**
     * Create inner horizontal X marker for Airfield Zone.
     * Drawn as two crossed line strokes added as rings to the same polygon,
     * so it is one geometry and inherits the polygon outline color.
     */
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            // Horizontal X: wider than tall to match the flat crossed-lines look.
            const halfW = cLenLimit * 1.6;
            const halfH = cLenLimit * 0.5;
            const cx = midPt.x;
            const cy = midPt.y;

            // Two diagonal strokes crossing at the center. Each is a zero-area
            // "there and back" ring [A, B, A] so it renders as a single clean
            // line within the polygon outline (same approach as NamedAreaOfInterest).
            result.addRing([[cx - halfW, cy - halfH], [cx + halfW, cy + halfH], [cx - halfW, cy - halfH]]);
            result.addRing([[cx - halfW, cy + halfH], [cx + halfW, cy - halfH], [cx - halfW, cy + halfH]]);
