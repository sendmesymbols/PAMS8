...
class Shapes {
  static createEchelon(type: string, point: Point, size: number): number[][][] {
    ...
    switch (type) {
      case '21':
        // Double fence wire pattern
        paths.push([
          [point.x - size, point.y - size/3],
          [point.x + size, point.y - size/3]
        ]);
        paths.push([
          [point.x - size, point.y + size/3],
...
interface DoubleFenceWireOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polyline;
}

export default class DoubleFenceWire extends Evented {
  ...
  public init(options: DoubleFenceWireOptions, marker: any): void {
  ...
  private createSymbol(drawEssentials: DrawEssentials): Polyline {
    try {
      ...
      for (let i = 0; i < resPts.length; i++) {
        const echelons = Shapes.createEchelon('21', resPts[i], cLenLimit);
