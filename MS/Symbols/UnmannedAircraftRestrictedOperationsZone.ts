...
export interface UnmannedAircraftRestrictedOperationsZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * UnmannedAircraftRestrictedOperationsZone class for Target Unmanned Aircraft Restricted Operations Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class UnmannedAircraftRestrictedOperationsZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.UnmannedAircraftRestrictedOperationsZone";
    private SID: string = "171200";
    private symName: string = "Unmanned Aircraft Restricted Operations Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "UnmannedAircraftRestrictedOperationsZone");
     ...
     */
    public init(options: UnmannedAircraftRestrictedOperationsZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createUARROZRings
                ? (Shapes as any).createUARROZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createUARROZ from the source JS
                const naiTxt = (Shapes as any).createUARROZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default UnmannedAircraftRestrictedOperationsZone;