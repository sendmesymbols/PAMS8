...
export interface FortifiedLineOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polyline;
    DRAW_TYPE?: number;
    [key: string]: any;
...
/**
 * Fortified Line drawn as repeating rectangular crenellations. The pattern
 * follows straight and curved control lines and remains one Polyline geometry.
 */
export class FortifiedLine {
    ...
    // Symbol properties
    public declaredClass: string = "MilitarySymbology.Symbols.FortifiedLine";
    private SID: string = "290900";
    private symName: string = "Fortified Line";
    ...
    // Crenellation count, raised width, height, and side.
    private _stepCount: number = 7;
    private _raisedWidthRatio: number = 0.6;
    private _stepHeightRatio: number = 0.6;
    private _stepSide: number = 1;
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "FortifiedLine");
     ...
     */
    public init(options: FortifiedLineOptions, marker: SimpleLineSymbol | SimpleFillSymbol): void {
     ...
     * Builds the base line, then converts it to a crenellated line.
     */
    ...
    /**
     * Convert a plain polyline into a repeating rectangular step pattern.
     */
    private buildFortifiedLine(linePts: number[][]): Polyline {
        ...
        const steps = Math.max(1, this._stepCount);
        const cell = total / steps;
        const stepHeight = cell * this._stepHeightRatio;
        const raisedHalf = cell * this._raisedWidthRatio / 2;

        const out: number[][] = [];
        out.push(this.pointAt(linePts, cum, 0));
        for (let k = 0; k < steps; k++) {
            const c = (k + 0.5) * cell;
            const raisedStart = c - raisedHalf;
            const raisedEnd = c + raisedHalf;
            out.push(this.pointAt(linePts, cum, raisedStart));
            out.push(this.offsetAt(linePts, cum, raisedStart, stepHeight));
            out.push(this.offsetAt(linePts, cum, raisedEnd, stepHeight));
            out.push(this.pointAt(linePts, cum, raisedEnd));
        }
    ...
    /** Point at s offset perpendicular to the local tangent. */
    private offsetAt(linePts: number[][], cum: number[], s: number, height: number): number[] {
        const total = cum[cum.length - 1];
        ...
        return [p[0] - ty * height * this._stepSide, p[1] + tx * height * this._stepSide];
    }
