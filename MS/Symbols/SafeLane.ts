...
export interface SafeLaneOptions {
    CTRL_PTS?: Point[];
...
export class SafeLane {
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "SafeLane");
    ...
    /**
     * Initialize the SafeLane drawing
     */
    public init(options: SafeLaneOptions, marker: SimpleLineSymbol): void {
        ...
        try {
            ...
            if (values && values.midPoints && Array.isArray(values.midPoints)) {
                for (let i = 0; i < values.midPoints.length; i++) {
                    ...
                    const slStrokes = (Shapes as any).createSL
                        ? (Shapes as any).createSL(values.midPoints[i].midPt.x, values.midPoints[i].midPt.y, cLenLimit, values.midPoints[i].midPt.spatialReference)
                        : [];
                    if (Array.isArray(slStrokes)) {
                        for (let j = 0; j <= slStrokes.length - 1; j++) {
                            const seg = slStrokes[j] as Point[];
                            if (seg && seg.length) {
                                result.addPath(seg.map(p => [p.x, p.y]));
                            }
                        }
