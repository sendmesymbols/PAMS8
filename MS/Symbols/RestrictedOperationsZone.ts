...
export interface RestrictedOperationsZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * RestrictedOperationsZone class for Target Restricted Operations Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class RestrictedOperationsZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.RestrictedOperationsZone";
    private SID: string = "171000";
    private symName: string = "Restricted Operations Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "RestrictedOperationsZone");
     ...
     */
    public init(options: RestrictedOperationsZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createROZRings
                ? (Shapes as any).createROZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createROZ from the source JS
                const naiTxt = (Shapes as any).createROZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default RestrictedOperationsZone;