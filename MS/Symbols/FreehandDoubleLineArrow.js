/**
 * Class Representing Freehand Double Line Arrow.
 *
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var FreehandDoubleLineArrow = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.FreehandDoubleLineArrow",
            SID: "000004",
            symName: "Freehand - Double Line Arrow",
            ...
            createSymbol: function (drawEssentials) {
                try {
                    ...
                    for (i = 0; i < pts.length; i++) {
                        ...
                        var shortenLeftPt = { x: -1 * length / 10 * Math.cos(k) + stPtCandidatePt.x, y: -1 * length / 5 * Math.sin(k) + stPtCandidatePt.y };
                        leftArray.push(shortenLeftPt);


                        //Shorten Right Array Point              
                        var shortenRightPt = { x: -1 * length / 10 * Math.cos(k) + endPtCandidatePt.x, y: -1 * length / 5 * Math.sin(k) + endPtCandidatePt.y };
                        rightArray.push(shortenRightPt);


                        //Shorten Middle Array Point
                        var shortenMiddlePt = { x: -1 * length / 10 * Math.cos(k) + pts[i].x, y: -1 * length / 10 * Math.sin(k) + pts[i].y };
                    ...
                    var LeftFlankPt = this.getFlankPts(shortenLeftPt, leftArray[0]);
                    var rightFlankPt = this.getFlankPts(shortenRightPt, rightArray[0]);
                    result.addPath([shortenLeftPt, LeftFlankPt[1], pts[pts.length - 1], rightFlankPt[0], shortenRightPt]);



                    //End of Create Arrow Head
            ...
            getFlankPts: function (firstPoint, lastPoint) {
                var cLenLimit;
                var baseLineLen = GeoTools._2PtLen(firstPoint, lastPoint) / 4;
                ...
                return [{ x: partialLen * Math.cos(k) + firstPoint.x, y: partialLen * Math.sin(k) + firstPoint.y }, { x: -1 * partialLen * Math.cos(k) + firstPoint.x, y: -1 * partialLen * Math.sin(k) + firstPoint.y }];
        ...
        return FreehandDoubleLineArrow;
    });
