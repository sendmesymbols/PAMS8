...
export class StartLine {
    ...
    // Symbol properties
    private SID: string = "140303";
    private symName: string = "Start Line";
     ...
     */
    public init(options: StartLineOptions, marker: SimpleLineSymbol | SimpleFillSymbol): void {
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
        try {
            ...
            // Add SL text markers at both ends
            const len = GeoTools._2PtLen(p1, p2) / 20;
            const k = GeoTools.angleInRadians(p1, p2);
            
            // SL marker at start point
            const pt1 = {
                x: -1 * len * Math.cos(k) + p1.x,
                y: -1 * len * Math.sin(k) + p1.y
            };

            // Create SL text markers (when Shapes.createSL is available)
            if ('createSL' in Shapes && typeof (Shapes as any).createSL === 'function') {
                const slPaths1 = (Shapes as any).createSL(pt1.x, pt1.y, len / 2, this.view.spatialReference);
                if (slPaths1 && Array.isArray(slPaths1)) {
                    slPaths1.forEach((path: any) => {
                ...
                // SL marker at end point
                const pt2 = {
                    x: len * Math.cos(k) + p2.x,
                    y: len * Math.sin(k) + p2.y
                };

                const slPaths2 = (Shapes as any).createSL(pt2.x, pt2.y, len / 2, this.view.spatialReference);
                if (slPaths2 && Array.isArray(slPaths2)) {
                    slPaths2.forEach((path: any) => {
                        if (path && Array.isArray(path)) {
                            result.addPath(path);
                        }
                    });
