...
export class StratAssyArea {
        ...
        try {
            ...
            if (Shapes && Shapes.createSAA) {
                try {
                    ...
                    this.createSimpleSAA(result, midPt, cLenLimit);
                }
            } else {
                // Fallback SAA creation
                this.createSimpleSAA(result, midPt, cLenLimit);
            }
    ...
    /**
     * Create simple SAA text as fallback
     */
    private createSimpleSAA(result: Polygon, midPt: Point, size: number): void {
        // Create simple "SAA" text representation using Shapes utility methods
        const letterHeight = size;
        const letterSpacing = size * 0.4;

        try {
            // Create S, A, A letters using Shapes utility
            const sPoints = Shapes.createS(midPt.x - letterSpacing * 1.5, midPt.y, letterHeight, midPt.spatialReference);
            ...
            const sCoords = sPoints.map(pt => [pt.x, pt.y]);
            const a1Coords = a1Points.map(pt => [pt.x, pt.y]);
            const a2Coords = a2Points.map(pt => [pt.x, pt.y]);

            result.addRing(sCoords);
            result.addRing(a1Coords);
            result.addRing(a2Coords);
        } catch (e) {
            console.log('Error creating SAA letters, using simple fallback');
            // Simple fallback - just add some basic shapes
            const letterWidth = size * 0.6;
            const spacing = size * 0.3;

            // Simple S shape (curved)
            const sShape: number[][] = [];
            const segments = 12;
            for (let i = 0; i <= segments; i++) {
                const t = i / segments;
                const angle = t * Math.PI;
                const x = midPt.x - letterWidth - spacing + (letterWidth/2) * Math.cos(angle);
                const y = midPt.y + (letterHeight/4) * Math.sin(angle);
                sShape.push([x, y]);
            }
