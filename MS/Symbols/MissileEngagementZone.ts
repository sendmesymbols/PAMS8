...
export interface MissileEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * MissileEngagementZone class for Target Missile Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class MissileEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.MissileEngagementZone";
    private SID: string = "171600";
    private symName: string = "Missile Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "MissileEngagementZone");
     ...
     */
    public init(options: MissileEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createMEZRings
                ? (Shapes as any).createMEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createMEZ from the source JS
                const naiTxt = (Shapes as any).createMEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default MissileEngagementZone;