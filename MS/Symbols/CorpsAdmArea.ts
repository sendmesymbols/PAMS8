...
export interface CorpsAdmAreaOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * CorpsAdmArea class for drawing Corps Administrative Area symbols
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers using createBAA method for "CAA" markers
 */
export class CorpsAdmArea {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.CorpsAdmArea";
    private SID: string = "150203";
    private symName: string = "Corps Adm Area";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "CorpsAdmArea");
     ...
     */
    public init(options: CorpsAdmAreaOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createCAARings
                ? (Shapes as any).createCAARings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createCAA from the source JS
                const caaRings = (Shapes as any).createCAA(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                if (caaRings && Array.isArray(caaRings)) {
                    for (let j = 0; j <= caaRings.length - 1; j++) {
                        if (caaRings[j]) {
                            result.addRing(caaRings[j]);
                        }
                    }
                }
            }
