...
/**
 * FwdAssemblyArea class for drawing Forward Assembly Area symbols
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers using createFAA method
 */
export class FwdAssemblyArea {
    ...
    // Symbol properties
    private SID: string = "150201";
    private symName: string = "Fwd Assy Area";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "FwdAssemblyArea");
     ...
     */
    public init(options: FwdAssemblyAreaOptions, marker: SimpleLineSymbol): void {
    ...
    /**
     * Create inner text markers for Forward Assembly Area
     */
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            // Try to use Shapes.createFAA if available
            if (Shapes && Shapes.createFAA) {
                try {
                    const faaRings = Shapes.createFAA(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                    if (faaRings && Array.isArray(faaRings)) {
                        for (let j = 0; j < faaRings.length; j++) {
                            const seg = faaRings[j];
...
export default FwdAssemblyArea;
