...
export interface ShortRangeAirDefenseEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * ShortRangeAirDefenseEngagementZone class for Target Short Range Air Defense Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class ShortRangeAirDefenseEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.ShortRangeAirDefenseEngagementZone";
    private SID: string = "171900";
    private symName: string = "Short Range Air Defense Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "ShortRangeAirDefenseEngagementZone");
     ...
     */
    public init(options: ShortRangeAirDefenseEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createSHORADEZRings
                ? (Shapes as any).createSHORADEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createSHORADEZ from the source JS
                const naiTxt = (Shapes as any).createSHORADEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
    ...
    private __onDrawEnd(geometry: Polygon, geoGeometry: Polygon, drawEssParam: DrawEssentials): void {
        this.events.emit("onDrawEnd", {
            ...
            marker: new SimpleFillSymbol({ style: "forward-diagonal", color: (this._lineSym as any).color, outline: (this._lineSym as any) })
        });
    }
...
export default ShortRangeAirDefenseEngagementZone;