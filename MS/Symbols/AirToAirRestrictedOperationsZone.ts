...
export interface AirToAirRestrictedOperationsZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * AirToAirRestrictedOperationsZone class for Target Air to Air Restricted Operations Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class AirToAirRestrictedOperationsZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.AirToAirRestrictedOperationsZone";
    private SID: string = "171100";
    private symName: string = "Air to Air Restricted Operations Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "AirToAirRestrictedOperationsZone");
     ...
     */
    public init(options: AirToAirRestrictedOperationsZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createAARROZRings
                ? (Shapes as any).createAARROZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createAARROZ from the source JS
                const naiTxt = (Shapes as any).createAARROZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default AirToAirRestrictedOperationsZone;