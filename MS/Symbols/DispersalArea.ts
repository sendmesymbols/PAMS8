...
export class DispersalArea {
  ...
  // Symbol properties
  public declaredClass: string = "MilitarySymbology.Symbols.DispersalArea";
  public SID: string = "150204";
  public symName: string = "Dispersal Area";
  ...
  constructor(view: MapView | SceneView, isLine: boolean = false) {
    ...
    this.events = new SymbolEvents(view, "DispersalArea");
   ...
   */
  public init(options: DispersalAreaOptions, marker: SimpleLineSymbol): void {
  ...
  private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
    try {
      ...
      // Legacy draws "DA" inside. Use Shapes.createDA (D and A strokes)
      const daStrokes = (Shapes as any).createDA(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
      if (daStrokes && Array.isArray(daStrokes)) {
        for (let j = 0; j < daStrokes.length; j++) {
          const stroke = daStrokes[j];
          if (stroke && stroke.length >= 2) {
            const ring = stroke.map((p: Point) => [p.x, p.y]);
            // Close minimal ring so it can be added to polygon geometry
            if (ring.length < 4) {
              const closed = [...ring, ring[ring.length - 1], ring[0]];
