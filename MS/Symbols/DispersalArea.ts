/**
 * Class Representing Dispersal Area.
 * @class
 * @author Abdul Razak
 */
...
class Shapes {
  static createDA(x: number, y: number, size: number, spatialRef: any): number[][][] {
    // Create DA (Dispersal Area) shape
...
interface DispersalAreaOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polygon;
  DRAW_TYPE?: number;
}

export default class DispersalArea extends Evented {
  ...
  public init(options: DispersalAreaOptions, marker: any): void {
  ...
  private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
    try {
      ...
      if (extent && extent.center) {
        ...
        const rings = Shapes.createDA(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
        for (let j = 0; j <= rings.length - 1; j++) {
          result.addRing(rings[j]);
        }
      }
      return result;
    } catch (e) {
