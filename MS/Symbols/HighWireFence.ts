...
export interface HighWireFenceOptions {
  CTRL_PTS?: Point[];
  GEOM?: Polyline;
  DRAW_TYPE?: number;
...
/**
 * HighWireFence class for Double Apron Fence symbol
 */
export class HighWireFence {
  ...
  constructor(view: MapView | SceneView, isLine: boolean = false) {
    ...
    this.events = new SymbolEvents(view, "HighWireFence");
   ...
   */
  public init(options: HighWireFenceOptions, marker: SimpleLineSymbol): void {
  ...
  private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
    try {
      ...
      // Create fence rails (left/right offsets) per legacy logic
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        // pt1/pt2 computed in legacy but not used for rails, so omitted

        leftArray.push([stPtCandidatePt.x, stPtCandidatePt.y]);
        rightArray.push([endPtCandidatePt.x, endPtCandidatePt.y]);
      }

      if (leftArray.length > 1) result.addPath(leftArray);
