...
export class ZoneOfResponsibility {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.ZoneOfResponsibility";
    public SID = "241901";
    public symName = "Zone of Responsibility";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "ZoneOfResponsibility");
     ...
     */
    public init(options: ZoneOfResponsibilityOptions, marker: SimpleLineSymbol): void {
    ...
    /**
     * Create inner text markers for Zone of Responsibility
     */
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createZORRings
                ? (Shapes as any).createZORRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createZOR from the source JS
                const zorTxt = (Shapes as any).createZOR(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                if (zorTxt && Array.isArray(zorTxt)) {
                    for (let j = 0; j <= zorTxt.length - 1; j++) {
                        if (zorTxt[j]) {
                            result.addRing(zorTxt[j]);
                        }
                    }
                }
            }
