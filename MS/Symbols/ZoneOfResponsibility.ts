...
class ZoneOfResponsibility {
    ...
    public init(options: ZoneOfResponsibilityOptions, marker: SimpleLineSymbol | SimpleFillSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        ...
        try {
            ...
            if (extent) {
                ...
                // Safe check for Shapes.createZOR method
                if (Shapes && typeof Shapes.createZOR === 'function') {
                    try {
                        const zor = Shapes.createZOR(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                        for (let j = 0; j <= zor.length - 1; j++) {
                            res.addRing(zor[j]);
                        }
                    } catch (e) {
                        console.log('Cannot create ZOR text');
                    }
                }
            }
            return res;
