...
export interface HighAltitudeMissileEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * HighAltitudeMissileEngagementZone class for Target High Altitude Missile Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class HighAltitudeMissileEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.HighAltitudeMissileEngagementZone";
    private SID: string = "171800";
    private symName: string = "High Altitude Missile Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "HighAltitudeMissileEngagementZone");
     ...
     */
    public init(options: HighAltitudeMissileEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createHIMEZRings
                ? (Shapes as any).createHIMEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createHIMEZ from the source JS
                const naiTxt = (Shapes as any).createHIMEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default HighAltitudeMissileEngagementZone;