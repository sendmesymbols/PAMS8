...
export class VitalArea {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.VitalArea";
    public SID = "242303";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "VitalArea");
     ...
     */
    public init(options: VitalAreaOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createVARings
                ? (Shapes as any).createVARings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createVA from the source JS
                const vaTxt = (Shapes as any).createVA(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
                if (vaTxt && Array.isArray(vaTxt)) {
                    for (let j = 0; j <= vaTxt.length - 1; j++) {
                        if (vaTxt[j]) {
                            result.addRing(vaTxt[j]);
                        }
                    }
                }
            }
