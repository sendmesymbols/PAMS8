...
export interface JointEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * JointEngagementZone class for Target Joint Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class JointEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.JointEngagementZone";
    private SID: string = "171500";
    private symName: string = "Joint Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "JointEngagementZone");
     ...
     */
    public init(options: JointEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createJEZRings
                ? (Shapes as any).createJEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createJEZ from the source JS
                const naiTxt = (Shapes as any).createJEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default JointEngagementZone;