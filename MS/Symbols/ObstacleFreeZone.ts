...
export interface ObstacleFreeZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * ObstacleFreeZone — identical to ObstacleZone (a green, toothed obstacle area) except the
 * teeth are notched INWARD (into the area) instead of pointing outward, plus the word "FREE"
 * written across the centre as geometry (like NamedAreaOfInterest's "NAI", so it renders in
 * the same green line symbol). The whole symbol is a single Polygon geometry.
 */
export class ObstacleFreeZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.ObstacleFreeZone";
    private SID: string = "270300";
    private symName: string = "Obstacle Free Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "ObstacleFreeZone");
    ...
    /**
     * Initialize the obstacle free zone drawing
     */
    public init(options: ObstacleFreeZoneOptions, marker: SimpleLineSymbol): void {
     ...
     * Same base + teeth as ObstacleZone (bezier / polygon / rect), but the teeth are notched
     * INWARD; then "FREE" is written across the centre. Single Polygon geometry.
     */
    private createSymbol(drawEssentials: DrawEssentials): Polygon | null {
        try {
            ...
            if (!base) return null;
            const toothed = this.applySawtooth(base);
            return this.addFreeText(toothed);
    ...
    /**
     * Write "FREE" across the centre of the polygon as letter strokes, added as rings so
     * they render with the polygon's (green obstacle) line symbol. Mirrors the way
     * NamedAreaOfInterest draws its inner "NAI" text. Uses the 3D-safe stroke variants.
     */
    private addFreeText(result: Polygon): Polygon {
        try {
            const extent = result.extent;
            if (!extent) return result;

            const cx = extent.center.x;
            const cy = extent.center.y;
            const sr = this.view.spatialReference;

            const dr = Math.min(extent.width, extent.height) * 0.10; // letter half-height
            if (!(dr > 0)) return result;

            const pitch = dr * 1.5;            // distance between letter origins
            const startX = cx - pitch * 1.5;   // centre the 4 letters (F R E E)

            const letterStrokes: Point[][] = [
                ...Shapes.createFStrokes(startX, cy, dr, sr),
                ...Shapes.createRStrokes(startX + pitch, cy, dr, sr),
                ...Shapes.createEStrokes(startX + pitch * 2, cy, dr, sr),
                ...Shapes.createEStrokes(startX + pitch * 3, cy, dr, sr),
            ];

            // Each stroke becomes a thin closed ring (NAI-style). strokesToSegments keeps
            // the curved R bowl as multiple 2-point segments so it survives 3D tessellation.
            const segments = Shapes.strokesToSegments(letterStrokes);
            ...
            console.log('Cannot create FREE text');
            return result;
        }
    }
...
export default ObstacleFreeZone;
