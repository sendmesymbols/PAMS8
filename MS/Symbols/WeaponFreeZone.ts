...
export interface WeaponFreeZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
export class WeaponFreeZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.WeaponFreeZone";
    private SID: string = "172000";
    private symName: string = "Weapon Free Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "WeaponFreeZone");
     ...
     */
    public init(options: WeaponFreeZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createWFZRings
                ? (Shapes as any).createWFZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createWFZ from the source JS
                const naiTxt = (Shapes as any).createWFZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
