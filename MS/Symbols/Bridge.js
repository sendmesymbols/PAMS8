...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var Bridge = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.Bridge",
            SID: "271100",
            symName: "Bridge - Gap",
            ...
            init: function (options, marker) {
                ...
                this._flap_angle = GeoTools.setDefault(options, "FLAP_ANGLE", this._flap_angle);
                ...
                if (options.hasOwnProperty("CTRL_PTS") && options.hasOwnProperty("BASE_LN_PTS") && options.hasOwnProperty("GEOM")) {
                    ...
                    drawEssentials = this.createDrawEssentials(lang.clone(options.CTRL_PTS), lang.clone(options.BASE_LN_PTS), this._tailFactor, this._flap_angle);
                    ...
                    if (options.hasOwnProperty("BASE_LN_PTS")) {
                        ...
                        throw "Control Points and Baseline Pts are required to create symbol non-interactively";
            ...
            createDrawEssentials: function (ctrlPts, baseLinePts, tailFactor, flagAngle) {
                var drawEssentials = new DrawEssentials();
                ...
                drawEssentials.FLAP_ANGLE = flagAngle;
            ...
            createSymbol: function (drawEssentials) {
                ...
                for (i = 0; i < pts.length; i++) {
                    ...
                    var shortenLeftPt = { x: -1 * length / 9 * Math.cos(angle) + stPtCandidatePt.x, y: -1 * length / 9 * Math.sin(angle) + stPtCandidatePt.y };
                    leftArray.push(shortenLeftPt);


                    //Shorten Right Array Point              
                    var shortenRightPt = { x: -1 * length / 9 * Math.cos(angle) + endPtCandidatePt.x, y: -1 * length / 9 * Math.sin(angle) + endPtCandidatePt.y };
                ...
                //Back Flaps
                paths = paths.concat(this._createStNotches(p1, shortenLeftPt, this._tailFactor, this._flap_angle, 0), p1);
                result.addPath(paths);


                paths = [];
                paths = paths.concat(this._createStNotches(p2, shortenRightPt, this._tailFactor, this._flap_angle, 1), p2);
                result.addPath(paths);
                //End of Back Flaps
                ...
                //Front Flaps
                paths = [];
                paths = paths.concat(this._createStNotches(shortenLeftPt, p1, this._tailFactor, this._flap_angle, 1), shortenLeftPt);
                result.addPath(paths);

                paths = [];
                paths = paths.concat(this._createStNotches(shortenRightPt, p2, this._tailFactor, this._flap_angle, 0), shortenRightPt);
                result.addPath(paths);
                //End of Front Flaps
            ...
            cleanUp: function () {
                ...
                drawEss = this.createDrawEssentials(lang.clone(this._points), lang.clone(this._baseLinePts), this._tailFactor, this._flap_angle);
            ...
            _createStNotches: function (firstPoint, lastPoint, len, angle, inner) {
                ...
                if (inner === 0) {                    
                    k += GeoTools.toRad(angle);
                    pt1 = { x: this._tailFactor * len * Math.cos(k) + firstPoint.x, y: this._tailFactor * len * Math.sin(k) + firstPoint.y };
                } else {
                    k -= GeoTools.toRad(angle);
                    pt1 = { x: -1 * this._tailFactor * len * Math.cos(k) + firstPoint.x, y: -1 * this._tailFactor * len * Math.sin(k) + firstPoint.y };
                }
