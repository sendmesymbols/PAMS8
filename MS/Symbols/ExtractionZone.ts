...
export interface ExtractionZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * ExtractionZone class for Extraction Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner "EZ" text marker
 */
export class ExtractionZone {
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "ExtractionZone");
     ...
     */
    public init(options: ExtractionZoneOptions, marker: SimpleLineSymbol): void {
    ...
    /**
     * Create inner "EZ" text marker for Extraction Zone.
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createEZRings
                ? (Shapes as any).createEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
