...
export interface LowAltitudeMissileEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * LowAltitudeMissileEngagementZone class for Target Low Altitude Missile Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class LowAltitudeMissileEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.LowAltitudeMissileEngagementZone";
    private SID: string = "171700";
    private symName: string = "Low Altitude Missile Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "LowAltitudeMissileEngagementZone");
     ...
     */
    public init(options: LowAltitudeMissileEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createLOMEZRings
                ? (Shapes as any).createLOMEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createLOMEZ from the source JS
                const naiTxt = (Shapes as any).createLOMEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default LowAltitudeMissileEngagementZone;