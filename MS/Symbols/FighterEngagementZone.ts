...
export interface FighterEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * FighterEngagementZone class for Target Fighter Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class FighterEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.FighterEngagementZone";
    private SID: string = "171400";
    private symName: string = "Fighter Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "FighterEngagementZone");
     ...
     */
    public init(options: FighterEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createFEZRings
                ? (Shapes as any).createFEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createFEZ from the source JS
                const naiTxt = (Shapes as any).createFEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default FighterEngagementZone;