...
export interface WeaponEngagementZoneOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polygon;
    DRAW_TYPE?: number;
...
/**
 * WeaponEngagementZone class for Target Weapon Engagement Zone symbol
 * Supports multiple drawing types: Bezier curve (1), Polygon (2), Rectangle (3)
 * Includes inner text markers
 */
export class WeaponEngagementZone {
    ...
    // Symbol properties
    public declaredClass = "MilitarySymbology.Symbols.WeaponEngagementZone";
    private SID: string = "171300";
    private symName: string = "Weapon Engagement Zone";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "WeaponEngagementZone");
     ...
     */
    public init(options: WeaponEngagementZoneOptions, marker: SimpleLineSymbol): void {
    ...
    private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
        try {
            ...
            const rings = (Shapes as any).createWEZRings
                ? (Shapes as any).createWEZRings(midPt.x, midPt.y, cLenLimit, midPt.spatialReference)
                : null;
            ...
            if (rings && Array.isArray(rings)) {
                ...
                // Fallback to legacy createWEZ from the source JS
                const naiTxt = (Shapes as any).createWEZ(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
...
export default WeaponEngagementZone;