...
import CpenPosition from "./CpenPosition.ts";

class TargetAreaOfInterest {
  public declaredClass = "MilitarySymbology.Symbols.TargetAreaOfInterest";
  public SID = "120201";
  public symName = "Tgt Area of Interest";
  ...
  private createInnerText(result: Polygon, firstPoint: Point, lastPoint: Point): Polygon {
    ...
    try {
      ...
      if (extent) {
        ...
        // Safe check for Shapes.createTAI method
        if (Shapes && typeof Shapes.createTAI === 'function') {
          const taiTxt = Shapes.createTAI(midPt.x, midPt.y, cLenLimit, midPt.spatialReference);
          for (let j = 0; j <= taiTxt.length - 1; j++) {
            res.addRing(taiTxt[j]);
          }
        }
      }
      return res;
