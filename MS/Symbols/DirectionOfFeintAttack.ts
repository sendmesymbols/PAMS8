...
export interface DirectionOfFeintAttackOptions {
    CTRL_PTS?: Point[];
    GEOM?: Polyline;
    DRAW_TYPE?: number;
    [key: string]: any;
...
/**
 * FriendlyDirOfSpAttk class for drawing Friendly Direction of Supporting Attack symbols on MapView or SceneView
 * Supports line and curve draw types with arrow head
 */
export class DirectionOfFeintAttack {
    ...
    // Symbol properties
    private SID: string = "140605";
    private symName: string = "Direction of Feint Attack";
    ...
    constructor(view: MapView | SceneView, isLine: boolean = false) {
        ...
        this.events = new SymbolEvents(view, "DirectionOfFeintAttack");
     ...
     */
    public init(options: DirectionOfFeintAttackOptions, marker: SimpleLineSymbol | SimpleFillSymbol): void {
    ...
    private createSymbol(drawEssentials: DrawEssentials): Polyline | null {
        try {
            ...
            // Add the normal solid arrow head and a larger punctured shell.
            // Every visible segment remains a path in this single Polyline.
            if (pts.length >= 2) {
                const tip = pts[pts.length - 1];
                const base = pts[pts.length - 2];
                const totalLength = GeoTools._2PtLen(pts[0], tip);
                const arrowLength = GeoTools.ArrowFlanksLen(totalLength, totalLength);
                const angle = GeoTools.angleInRadians(base, tip);

                const solidHead = Shapes.createSimpleArrowHead(tip, base, arrowLength);
                result.addPath(solidHead);

                const shellTip = new Point({
                    x: tip.x + arrowLength * 0.45 * Math.cos(angle),
                    y: tip.y + arrowLength * 0.45 * Math.sin(angle),
                    spatialReference: this.view.spatialReference
                });
                const shellHead = Shapes.createSimpleArrowHead(
                    shellTip,
                    tip,
                    arrowLength * 1.55
                );

                this.addPuncturedArrowArm(result, shellHead[1], shellHead[0]);
                this.addPuncturedArrowArm(result, shellHead[1], shellHead[2]);
            }
            return result;
    ...
    /**
     * Add one shell arm as three geometry segments separated by gaps.
     */
    private addPuncturedArrowArm(result: Polyline, tip: number[], end: number[]): void {
        const dx = end[0] - tip[0];
        const dy = end[1] - tip[1];
        ...
        segments.forEach(([startRatio, endRatio]) => {
            result.addPath([
                [tip[0] + dx * startRatio, tip[1] + dy * startRatio],
                [tip[0] + dx * endRatio, tip[1] + dy * endRatio],
            ]);
        });
    }
...
export default DirectionOfFeintAttack; 
