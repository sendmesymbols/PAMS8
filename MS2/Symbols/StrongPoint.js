...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var StrongPoint = declare([Evented], { declaredClass: "MilitarySymbology.Symbols.StrongPoint",
          SID : "151203",
          symName :"Strong Pt",
        ...
        createStrongPts : function(pts, gap_ratio, center_pt, result, type) {
            //Create Strong Points
            var paths = [];
            var firstPoint = new Point(pts[0].x, pts[0].y, this.map.spatialReference);
            ...
            //var resPts = GeoTools.getDashPts(pts, [10, 10]);
            var k;
            for (var i = 0; i < resPts.length; i+=2) {
                k = GeoTools.twoPtsAngle(resPts[i], center_pt);
                //k+= 3.14159;
                k+= Math.PI;
                result.addPath([new Point(cLenLimit * Math.cos(k) + resPts[i].x, cLenLimit * Math.sin(k) + resPts[i].y, this.map.spatialReference),
                 resPts[i]]);
