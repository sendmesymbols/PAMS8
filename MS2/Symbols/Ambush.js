...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
  ...
  function (declare, lang, Array,
    ...
    var Ambush = declare([Evented], {
      declaredClass: "MilitarySymbology.Symbols.Ambush",
      SID: "141700",
      symName: "Ambush",
      ...
      createSymbol: function (drawEssentials) {
        try {
          ...
          if (pts.length === 2) {
            ...
            var length = GeoTools._2PtLen(endPt, result.getPoint(0, 30)) / 10;
            var teethSize = length * GeoTools.setDefault(drawEssentials, "TEETH_SIZE", this._teethSize);
            var angle = GeoTools.angleInRadians(centerPt, result.getPoint(0, 30));
            var teethGap = GeoTools.setDefault(drawEssentials, "TEETH_GAP", this._teethGap);
            for (var i = teethGap; i < 60; i += teethGap) {
              result.addPath(this.createTeeth(result.getPoint(0, i), angle, teethSize));
            }
            ...
            paths.push(result.getPoint(0, 30));
