...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var Canalize = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.Canalize",
			SID: "340400",
			symName: "Canalize",
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					result.addPath(Shapes.createCC(values.midPoint.x, values.midPoint.y, cLenLimit, this.map.spatialReference));
					...
					result.addPath(this._flaps(leftArray[leftArray.length - 1], GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(stPtCandidatePt, endPtCandidatePt)),
						GeoTools.angleInRadians(leftArray[leftArray.length - 2], leftArray[leftArray.length - 1]), 1));

					result.addPath(this._flaps(rightArray[rightArray.length - 1], GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(stPtCandidatePt, endPtCandidatePt)),
						GeoTools.angleInRadians(rightArray[rightArray.length - 2], rightArray[rightArray.length - 1]), 0));
