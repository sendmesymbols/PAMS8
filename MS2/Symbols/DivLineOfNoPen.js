/**
 * Class Representing Div Line Of No Penetration.
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var DivLineOfNoPen = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.DivLineOfNoPen",
			SID: "141900",
			symName: "Div Line of No Pen",
			...
			init: function (options, marker) {
				...
				var pupleColor = new color('#800080');
				this._lineSym.color = pupleColor;
				...
				if (options.hasOwnProperty("CTRL_PTS") && options.hasOwnProperty("GEOM")) {
					...
					drawEssentials = this.createDrawEssentials(lang.clone(options.CTRL_PTS), this._opacity);
			...
			createDrawEssentials: function (ctrlPts, opacity) {
				var drawEssentials = new DrawEssentials();
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					var paths = Shapes.createDLNP(pt1.x, pt1.y, len / 2, map.spatialReference);
					...
					var paths = Shapes.createDLNP(pt2.x + (len / 0.5), pt2.y, len / 2, map.spatialReference);
					for (var i = 0; i < paths.length; i++) {
						result.addPath(paths[i]);
					}
			...
			cleanUp: function () {
				...
				drawEss = this.createDrawEssentials(lang.clone(this._points), this._opacity);
