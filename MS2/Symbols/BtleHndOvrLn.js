/**
 * Class Representing Battle Handover Line.
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var BtleHndOvrLn = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.BtleHndOvrLn",
			SID: "141902",
			symName: "Battle Handover Line (BHOL)",
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					var paths = Shapes.createBHOL(pt1.x, pt1.y, len / 2, this.map.spatialReference, this.map);
					...
					var paths = Shapes.createBHOL(pt2.x + (len / 0.5), pt2.y, len / 2, this.map.spatialReference, this.map);
					for (var i = 0; i < paths.length; i++) {
						result.addPath(paths[i]);
					}
