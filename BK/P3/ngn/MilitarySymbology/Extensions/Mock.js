...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (_declare, _lang, Array, _connect, color, _window, _has, _keys, _domconstruct, _domstyle, esriKernel, esriSniff, Toolbar, SimpleMarker, SimpleLine, SimpleFill, graphic, jsonUtility, _webMercatorUtils, _Polyline, _Polygon, _Multipoint, _Rect, _dojoEsrijsapi) {
        var Mock = _declare(Toolbar, {
            declaredClass: "MilitarySymbology.Extensions.Mock",
            ...
            activate: function (geometryType, options) {
                ...
                var mapHolder = this.map, connectionHandler = _connect.connect, Draw = Mock;
            ...
            deactivate: function () {
                ...
                switch (this._geometryType) {
                    ...
                    case Mock.TRIANGLE:
                    case Mock.ARROW:
                    case Mock.LEFT_ARROW:
                    case Mock.RIGHT_ARROW:
                    case Mock.UP_ARROW:
                    case Mock.DOWN_ARROW:
                    case Mock.RECTANGLE:
                    case Mock.LINE:
                    case Mock.EXTENT:
                    case Mock.FREEHAND_POLYLINE:
                    case Mock.FREEHAND_POLYGON:
                        this._activateMapTools(!0, !1, !1, !0);
                        break;
                    case Mock.POLYLINE:
                    case Mock.POLYLINEEX:
                    case Mock.POLYGON:
                    case Mock.MULTI_POINT:
                        this._dblClickZoom && mapHolder.enableDoubleClickZoom();
                        break;
                    case Mock.CURVE:
                    case Mock.BEZIER_CURVE:
                    case Mock.BEZIER_POLYGON:
                    case Mock.FREEHAND_ARROW:
                        this._activateMapTools(!0, !1, !1, !0);
                        this._dblClickZoom && mapHolder.enableDoubleClickZoom();
                        break;
                }
            ...
            finishDrawing: function () {
                var returnGeometry, TempPoints = this._points, SpatialReference = this.map.spatialReference, Draw = Mock, PointCollection = TempPoints.slice(0, TempPoints.length);
                switch (this._geometryType) {
                    case Draw.POLYLINE:
            ...
            _onMouseDownHandler: function (clickPoint) {
                ...
                var start = snapPoint || clickPoint.mapPoint, Draw = Mock;
                var map = this.map;
                ...
                switch (this._geometryType) {
                    ...
                    case Mock.CURVE:
                        if (this._curvePt1 && this._curvePt2) {
                            console.log("_onMouseMoveHandler: function (inputPoint)");


                            var circle = this._circleMock(this.map.toScreen(this._curvePt1), this.map.toScreen(this._curvePt2), this.map.toScreen(candidatePoint));
                            if (circle.radius > 0) {
            ...
            _controlPointsUpdates: function (type, graphic, controlPoints) {
                switch (type) {
                    ...
                    case Mock.BEZIER_CURVE:
                        ...
                        break;
                    case Mock.CURVE:
                        var circle = this._circleMock(this.map.toScreen(controlPoints[0]), this.map.toScreen(controlPoints[1]), this.map.toScreen(controlPoints[2]));
                        if (circle.radius > 0) {
            ...
            _onMouseDragHandler: function (inputPt) {
                if (_has("esri-touch") && !this._points.length) inputPt.preventDefault();
                    ...
                    var Draw = Mock, firstPtScreen = map.toScreen(firstPt);
            ...
            _onMouseUpHandler: function (b) {
                if (this._dragged) {
                    ...
                    a = a || b.mapPoint;
                    var d = this.map.spatialReference, g = Mock, k;
                    switch (this._geometryType) {
                        case g.CIRCLE:
                        case g.ELLIPSE:
                        case g.TRIANGLE:
            ...
            _onClickHandler: function (clickPoint) {
                ...
                var map = this.map;
                var screenPt = map.toScreen(clickPoint), Draw = Mock;
            ...
            _onDblClickHandler: function (clickPoint) {
                ...
                var geometry, _points = this._points, spatialReference = this.map.spatialReference, Draw = Mock;
                ...
                switch (this._geometryType) {
                    ...
                    case Draw.BEZIER_CURVE:
                        ...
                        if (_points.length > 2) {
                            ...
                            var controlPts = _lang.clone(this._points);
                            geometry.controlPoints = controlPts;
                            geometry.MocktendType = "beziercurve";
                        }
                    ...
                    case Draw.BEZIER_POLYGON:
                        if (_points.length > 2) {
                            ...
                            var controlPts = _lang.clone(this._points);
                            geometry.controlPoints = controlPts;
                            geometry.MocktendType = "bezierpolygon";
                        }
                        break;
                    case Draw.FREEHAND_ARROW:
                        if (_points.length > 1) {
                            ...
                            var controlPts = _lang.clone(this._points);
                            geometry.controlPoints = controlPts;
                            geometry.MocktendType = "freehandarrow";
                        }
                        break;
                    case Draw.POLYLINEEX:
                        if (_points.length > 1) {
            ...
            _circleMock: function (pt1, pt2, pt3) {
                var i;
                ...
                console.log("_circleMock: function (pt1, pt2, pt3)");
            ...
            _determinantMock: function (a, n) {
                ...
                if (n == 2)                                // terminate recursion
                {
                    ...
                    for (j1 = 0; j1 < n; j1++)            // do each column
                    {
                        ...
                        // sum (+/-)cofactor * minor
                        d = d + Math.pow(-1.0, j1) * a[0][j1] * this._determinantMock(m, n - 1);
                    }
                }
            ...
            CreateCircleSegmentFromThreePoints: function (circle, pt1, pt2, pt3, numberOfPts, map) {
                ...
                result.addPath(path);
                console.log("CreateCircleSegmentFromThreePoints: function (circle, pt1, pt2, pt3, numberOfPts, map)");
        ...
        _has("extend-esri") && _lang.setObject("toolbars.Draw", Mock, esriKernel);
        return Mock
    });
