        ...
        var Box = declare(null, {
                    ...
                    _draw: function() {
                        if (this._graphic.getDojoShape()) {
                            ...
                            var map = this._map,
                                scratchGL = this._toolbar._scratchGL,
                                boxCoords = this._getBoxCoords(),
                                polyLine = new Polyline(map.spatialReference),
                            ...
                            this._anchors ?
                                ...
                                if (this._scale || !(8 > index)) {
                                    ...
                                    this._anchors.push({
                                        graphic: d,
                                        moveable: this._getMoveable(d, index)
                                    })
                                }
                            }, this)); 
				       if (this._controlPoints)
				       {
                        console.log("if (this._controlPoints) in Draw");
					       if (this._cpAnchors)
					       {
            ...
            _getBoxCoords: function(useScreen) {
                var map = this._map,
                    screenBoundingBox = this._getTransformedBoundingBox(this._graphic),
                    a = [],
                ...
                }), screenBoundingBox.y -= this._toolbar._options.rotateHandleOffset, useScreen || (screenBoundingBox = map.toMap(screenBoundingBox)), a.push([screenBoundingBox.x, screenBoundingBox.y]));
                return a;
            },
            ...
            _firstMoveHandler: function(b) {
                ...
                if (8 === index) middeScreen = matrix.multiplyPoint(matrix.invert(transform), screenBbox[1]), this._startLine = [this._centerCoord, middeScreen], this._moveLine = lang.clone(this._startLine);
                else if (middeScreen = matrix.multiplyPoint(matrix.invert(transform), screenBbox[index]), transform = matrix.multiplyPoint(matrix.invert(transform), screenBbox[(index + 4) % 8]), this._firstMoverToCenter = Math.sqrt((middeScreen.x - this._centerCoord.x) * (middeScreen.x - this._centerCoord.x) + (middeScreen.y - this._centerCoord.y) * (middeScreen.y - this._centerCoord.y)), this._startBox = transform, this._startBox.width = screenBbox[4].x - screenBbox[0].x, this._startBox.height = screenBbox[4].y - screenBbox[0].y, this._moveBox =
                    lang.clone(this._startBox), this._xfactor = middeScreen.x > transform.x ? 1 : -1, this._yfactor = middeScreen.y > transform.y ? 1 : -1, 1 === index || 5 === index) this._xfactor = 0;
                else if (3 === index || 7 === index) this._yfactor = 0;
