/**
 * Class Representing Support By Fire Position.
 *
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var SupportByFirePosition = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.SupportByFirePosition",
            SID: "152100",
            symName: "BOF",
		 ...
		 */
            ...
            init: function (options, marker) {
                ...
                if (options.hasOwnProperty("CTRL_PTS") && options.hasOwnProperty("BASE_LN_PTS") && options.hasOwnProperty("GEOM")) {
                    ...
                    drawEssentials = this.createDrawEssentials(lang.clone(options.CTRL_PTS), lang.clone(options.BASE_LN_PTS), this.backLineDist,
                        this.backLineAngle, this.frontLineAgle);
            ...
            createDrawEssentials: function (ctrlPts, baseLinePts, backLineDistRatio, backLineAngleRatio, frontLineAngleRatio) {
                var drawEssentials = new DrawEssentials();
                ...
                drawEssentials.BK_LN_DIST_RATIO = backLineDistRatio;
                drawEssentials.BK_LN_ANGL_RATIO = backLineAngleRatio;
            ...
            createSymbol: function (drawEssentials) {
                try {
                    ...
                    var pts, backLineDist, backLineAngle, frontLineAgle;
                    ...
                    backLineDist = GeoTools.setDefault(drawEssentials, "BK_LN_DIST_RATIO", 5);
                    backLineAngle = GeoTools.setDefault(drawEssentials, "BK_LN_ANGL_RATIO", 5);
                    frontLineAgle = GeoTools.setDefault(drawEssentials, "FRNT_LN_ANGL_RATIO", 5);
                    ...
                    for (var i = 0; i < pts.length; i++) {
                        ...
                        //var length = GeoTools._2PtLen(midPt, lastPoint);                        
                        //var angle = GeoTools.angleInRadians(midPt, lastPoint);
                    ...
                    result.addPath(this._arrowHead(pt1, GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(stPtCandidatePt, endPtCandidatePt)),
                        GeoTools.angleInRadians(leftArray[leftArray.length - 2],
                            pt1)));
                    result.addPath(this._arrowHead(pt2, GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(stPtCandidatePt, endPtCandidatePt)),
                    ...
                    length = GeoTools._2PtLen(midPt, lastPoint);
                    angle = GeoTools.angleInRadians(midPt, lastPoint);
                    length = length / backLineDist;

                    var stPtBackPt = new Point(p1.x - length * Math.cos(angle), p1.y - length * Math.sin(angle), this.map.spatialReference);
                    var endPtBackPt = new Point(p2.x - length * Math.cos(angle), p2.y - length * Math.sin(angle), this.map.spatialReference);

                    len = length / backLineAngle;
                    angle = GeoTools.angleInRadians(stPtBackPt, endPtBackPt);

                    var backPt1 = new Point(-1 * len * Math.cos(angle) + stPtBackPt.x, -1 * len * Math.sin(angle) + stPtBackPt.y, this.map.spatialReference);
                    var backPt2 = new Point(len * Math.cos(angle) + endPtBackPt.x, len * Math.sin(angle) + endPtBackPt.y, this.map.spatialReference);

                    paths = [];
                    paths = paths.concat(p1, backPt1);
                    result.addPath(paths);

                    paths = [];
                    paths = paths.concat(p2, backPt2);
                    result.addPath(paths);
            ...
            _onMMoveHdler: function (inputPoint) {
                ...
                drawEssentials.FRNT_LN_ANGL_RATIO = this.frontLineAgle;
            ...
            cleanUp: function () {
                ...
                drawEss = this.createDrawEssentials(lang.clone(this._points), lang.clone(this._baseLinePts), this.backLineDist,
                    this.backLineAngle, this.frontLineAgle);
            ...
            _arrowHead: function (candidatePoint, length, angle) {
                ...
                /*
					var rightWing = new Point(candidatePoint.x + length * Math.cos(GeoTools.toRad(angle)), 
          				candidatePoint.y + length * Math.sin(GeoTools.toRad(angle)), this.map.spatialReference);
