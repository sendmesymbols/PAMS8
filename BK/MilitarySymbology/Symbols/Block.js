...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var Block = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.Block",
			SID: "340100",
			symName: "Block",
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					for (var i = 0; i < values.midPoints.length; i++) {
						...
						result.addPath(Shapes.createB(values.midPoints[i].midPt, cLenLimit, 40));
			...
			_onMMoveHdler: function (inputPoint) {
				...
				/*
				//Saved Code for other symbols
				
				var lastPoint = this._points[this._points.length - 1];
				var firstPoint = midPt;
				var candidatePoint = inputPoint.mapPoint;
				...
				var paths = [];
				var result = new _Polyline(this.map.spatialReference);
				var stPt, midPt, endPt;
	
				stPt = this._tGraphic.attributes.startPt;
				endPt = this._tGraphic.attributes.endPt;
				midPt = this._tGraphic.attributes.midPt;
	
	
				midPt = midPt;
	
				var len = GeoTools._2PtLen(midPt, endPt);
				var k = Math.atan((midPt.y - candidatePoint.y) / (midPt.x - candidatePoint.x));
				switch (GeoTools.twoPtsRelationShip(midPt, candidatePoint))
				{
					case "ne":
						k += Math.PI / 2;
						break;
				...
				//Find distance between candidatePoint and Mid Point
				var length = GeoTools._2PtLen(midPt, candidatePoint);
				var angle = GeoTools.angleInRadians(midPt, candidatePoint);
				...
				paths = [];
				paths = paths.concat(this._arrowHead(stPtCandidatePt, length / 10, angle), stPtCandidatePt, p1);
				result.addPath(paths);
	
				paths = [];
				paths = paths.concat(this._arrowHead(endPtCandidatePt, length / 10, angle), endPtCandidatePt, p2);
				result.addPath(paths);
