/**
 * Class Representing Axis Of Advance Feint.
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
  ...
  function (declare, lang, Array,
    ...
    var AxisOfAdvanceFeint = declare([Evented], {
      declaredClass: "MilitarySymbology.Symbols.AxisOfAdvanceFeint",
      SID: "151406",
      symName: "Axis Of Advance For Feint",
      ...
      createSymbol: function (drawEssentials) {
        try {
          ...
          if (pts.length <= 2) {
            ...
            //Line in front of Arrow


            var pts = GeoTools._fracturePts(values.newLeftOuterPt, values.newCandiadatePt, 10, this.map.spatialReference);
            GeoTools.addAllRings(pts.geometry.paths, result);



            pts = GeoTools._fracturePts(values.newRightOuterPt, values.newCandiadatePt, 10, this.map.spatialReference);
            GeoTools.addAllRings(pts.geometry.paths, result);
      ...
      CreateArrowHeadPathEx: function (pt1, candidatePt, pt2, totalLen, headPercentage, headAngle, straight) {
        ...
        var newCandiadatePt = { x: candidatePt.x + headSideLen / 5 * Math.cos(angle), y: candidatePt.y + headSideLen / 5 * Math.sin(angle) };
        var newLeftOuterPt = { x: leftOuterPt.x + headSideLen / 5 * Math.cos(angle), y: leftOuterPt.y + headSideLen / 5 * Math.sin(angle) };
        var newRightOuterPt = { x: rightOuterPt.x + headSideLen / 5 * Math.cos(angle), y: rightOuterPt.y + headSideLen / 5 * Math.sin(angle) };
        ...
        return { rings: ring, newLeftOuterPt: newLeftOuterPt, newCandiadatePt: newCandiadatePt, newRightOuterPt: newRightOuterPt };
