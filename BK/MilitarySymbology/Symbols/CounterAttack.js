/**
 * Class Representing Counter Attack.
 *
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var CounterAttack = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.CounterAttack",
            SID: "340600",
            symName: "Counter Attk",
            ...
            createSymbol: function (drawEssentials) {
                try {
                    ...
                    if (pts.length <= 2) {
                        ...
                        var midPt = GeoTools.getMidPoint(firstPoint, lastPoint);

                        var cLenLimit;
                        var baseLineLen = GeoTools._2PtLen(p1, p2);
                        cLenLimit = baseLineLen / 2;
                        if (cLenLimit > baseLineLen / 3.6) cLenLimit = baseLineLen / 3.6;
                        var values = Shapes.CATK(midPt.x, midPt.y, cLenLimit);
                        for (var i = 0; i < values.length; i++) {
                            result.addRing(values[i]);
                        }
                        ...
                        //var indexRight = Math.round(rightArray.length / pts.length);
                        ...
                        var cLenLimit; 
                        var baseLineLen = GeoTools._2PtLen(leftArray[leftArray.length -1], rightArray[rightArray.length -1]);

                        GeoTools.displayPoint(this.map, leftArray[pts[pts.length-2]]);
                        GeoTools.displayPoint(this.map, rightArray[pts[pts.length-2]]);

                        cLenLimit = baseLineLen / 2;
                        if(cLenLimit > baseLineLen / 3.6) cLenLimit = baseLineLen / 3.6;
                        result.addRing(Shapes.createC(midPt, cLenLimit, 40));
                        */
