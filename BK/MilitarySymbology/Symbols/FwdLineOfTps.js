...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var FwdLineOfTps = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.FwdLineOfTps",
			SID: "140101",
			symName: "Fwd Line of Troops",
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					var k = GeoTools.angleInRadians(pts[pts.length - 1], pts[pts.length - 2]);


					var gapRatio = GeoTools._2PtLen(pts[0], pts[pts.length - 1]);
					gapRatio = gapRatio / 20;
					var cLenLimit, echelons;
					var baseLineLen = GeoTools._2PtLen(pts[0], pts[pts.length - 1]) / 6.58;
					cLenLimit = baseLineLen / 6.58;
					if (cLenLimit > baseLineLen / 3.6) cLenLimit = baseLineLen / 3.6;
					var resPts = GeoTools.getDashPts(pts, [gapRatio, gapRatio]);

					for (var i = 1; i < resPts.length - 1; i++) {
						echelons = Shapes.createFLOTHalfCircle(resPts[i], GeoTools.angleInRadians(resPts[i], resPts[i - 1]), cLenLimit);
