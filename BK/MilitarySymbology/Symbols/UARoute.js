...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var UARoute = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.UARoute",
            SID: "170700",
            symName: "UAV Route",
            ...
            createSymbol: function (drawEssentials) {
                try {
                    ...
                    var gapLen = GeoTools._2PtLen(endPt, stPt);
                    var text;
                    for (i = 0; i < pts.length; i++) {
                        ...
                        //Create Circles
                        ///Circles
                        result.addPath(this.createACP(pts[i], gapLen / 2));
                        //Circles

                        text = Shapes.createACP(pts[i].x, pts[i].y, gapLen / 10, pts[i].spatialReference);
                    ...
                    text = Shapes.createACP(middleArray[0].x, middleArray[0].y, gapLen / 10, middleArray[0].spatialReference);
                    for (var j = 0; j <= text.length - 1; j++) {
                        result.addPath(text[j]);
                    }
                    ...
                    for (var i = 0; i < values.midPoints.length; i++) {
                        ...
                        var text = Shapes.createUA(values.midPoints[i].midPt.x, values.midPoints[i].midPt.y, cLenLimit, values.midPoints[i].midPt.spatialReference);
                        for (var j = 0; j <= text.length - 1; j++) {
                            result.addPath(text[j]);
                        }
            ...
            createACP: function (pt, radius) {
                var paths = [];
                paths = Shapes.createCircle(pt, radius, 60);
