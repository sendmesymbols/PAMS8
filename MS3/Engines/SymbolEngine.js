 ...
 * version 0.7
 * Dated: 23 Nov 2018
 * Added cap to indicate what is being drawn.
 * Dated: 03 Jul 2019
 * Added cap to transform geom
 */
...
define(["dojo/_base/declare", "dojo/Evented", "esri/toolbars/draw", "esri/toolbars/edit",
    ...
    "esri/SpatialReference","esri/symbols/SimpleMarkerSymbol",
        ...
        function (declare, Evented, Draw, Edit, SimpleFillSymbol,
        Color, Graphic, SIDC,
        json, Mapper, AnnotationEngine,
                Point, Polyline, Polygon, SpatialReference,SimpleMarkerSymbol, 
        symData, settings) {
        ...
        var SymbolEngine = declare([Evented], {
            ...
            */
                ...
                try {
                    ...
                    if (this.currentSymbol.SymGeoType === "Point" || this.currentSymbol.SymGeoType === "FPoint") {
                        ...
                        */
                        ...
                        if (extraSettings !== undefined) {
                            if (this.currentSymbol.SymGeoType === "Point") {
                                ...
                                if (extraSettings.hasOwnProperty('opacity')) {
                                        if (!(drawEssentials.SIDC === "000110" || drawEssentials.SIDC === "000111")){
                        ...
                        drawEssentials.labelOptions = labelOptions;
            ...
            drawSymEnd: function (oldThis, event) {
                ...
                var symbol;
                    var symGeom, textGeom;
                    if (event.geometry.type === "polyline" || event.geometry.type === "point" || event.geometry.type === "polygon") {
                    symbol = event.marker;
                        symGeom = event.geometry;                        

                        if(event.drawEssentials.SYM_NAME === "Freehand - TextBox"){
                            if(event.textPtGeom){
                                textGeom = event.textPtGeom;
                            }else{
                                textGeom = symGeom.getExtent().getCenter();
                            }   
                        }else{
                            textGeom = event.geometry;
                        }                        
                    }else {
                ...
                if (graphic.drawEssentials.hasOwnProperty('ISFHAND')) {
                    ...
                    if(oldThis.labelOptions){
                event.drawEssentials.labelOptions = oldThis.labelOptions;
                    }


                    var options = oldThis.getOpacityValue(graphic);
                    
                    var textSize = 15;
                    if(oldThis.labelOptions){
                        if(oldThis.labelOptions.textSize){
                            textSize = oldThis.labelOptions.textSize;
                }
                    }
                    AnnotationEngine.annotate(oldThis.textGraphicsLayer, textGeom, event.drawEssentials.AMPLIFIER, event.drawEssentials, graphic.id,textSize , isFreeHand, oldThis.labelOptions, options);
                    if (event.drawEssentials.hasOwnProperty('opacity'))
                        delete event.drawEssentials.opacity;
