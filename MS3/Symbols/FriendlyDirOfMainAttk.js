/**
 * Class Representing FriendlyDirOfMainAttk.
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
    ...
    function (declare, lang, Array,
        ...
        var FriendlyDirOfMainAttk = declare([Evented], {
            declaredClass: "MilitarySymbology.Symbols.FriendlyDirOfMainAttk",
            SID: "140602",
            symName: "Friendly Direction of Main Attk",
            ...
            createSymbol: function (drawEssentials) {
                try {
                    ...
                    var len = GeoTools._2PtLen(pts[0], pts[pts.length - 1]);
                    path = [];
                    var len = len / 40;
                    var k = GeoTools.angleInRadians(pts[pts.length - 2], pts[pts.length - 1]);
                    var newMp = { x: len * Math.cos(k) + pts[pts.length - 1].x, y: len * Math.sin(k) + pts[pts.length - 1].y };
                    ...
                    //Arrow Head
                    var arrow1 = Shapes.arrowHead(newMp, GeoTools.ArrowFlanksLen(GeoTools._2PtLen(pts[pts.length - 2], pts[pts.length - 1]), GeoTools._2PtLen(pts[pts.length - 2], pts[pts.length - 1])),
                        GeoTools.angleInRadians(pts[pts.length - 2],
                            pts[pts.length - 1]));

                    var arrow2 = Shapes.arrowHead(pts[pts.length - 1], GeoTools.ArrowFlanksLen(GeoTools._2PtLen(pts[pts.length - 2], pts[pts.length - 1]), GeoTools._2PtLen(pts[pts.length - 2], pts[pts.length - 1])),
                        GeoTools.angleInRadians(pts[pts.length - 2],
                            pts[pts.length - 1]))
                    ...
                    path = [];
                    path = path.concat(arrow1[0], arrow2[0]);
                    result.addPath(path);


                    path = [];
                    path = path.concat(arrow1[arrow1.length - 1], arrow2[arrow2.length - 1]);
                    result.addPath(path);
