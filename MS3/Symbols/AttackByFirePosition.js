/**
 * Class Representing SupportByFirePosition.
 *
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var AttackByFirePosition = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.SupportByFirePosition",
			SID: "152000",
			symName: "Attk by Fire Posn",
			...
			createDrawEssentials: function (ctrlPts, baseLinePts, backLineDistRatio, BackLineAngleRatio) {
				var drawEssentials = new DrawEssentials();
			...
			baseLineDrawEnd: function (evt) {
				...
				this._onBaseLineProgress.remove();
				this._onBaseLineClick.remove();
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					//Arrows
					//length = GeoTools._2PtLen(midPt, pts[pts.length -1]);

					//result.addPath(this._arrowHead(pt1, length / 10, GeoTools.angleInRadians(leftArray[leftArray.length -2], pt1)));
					result.addPath(this._arrowHead(pts[pts.length - 1], GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(p1, p2)), GeoTools.angleInRadians(leftArray[leftArray.length - 2], pts[pts.length - 1])));
					/*
				  result.addPath(this._arrowHead(pt2, length / 10, 
