...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
	...
	function (declare, lang, Array,
		...
		var DisruptObstacleEffect = declare([Evented], {
			declaredClass: "MilitarySymbology.Symbols.DisruptObstacleEffect",
			SID: "270502",
			symName: "Disrupt / Obs Effect",
			...
			createSymbol: function (drawEssentials) {
				try {
					...
					for (i = 0; i < pts.length; i++) {
						...
						var shortenRightPt = { x: -1 * length / 5 * Math.cos(k) + endPtCandidatePt.x, y: -1 * length / 5 * Math.sin(k) + endPtCandidatePt.y };
					...
					result.addPath(Shapes.arrowHead(middleArray[middleArray.length - 1], GeoTools.ArrowFlanksLen(GeoTools._2PtLen(midPt, pts[pts.length - 1]), GeoTools._2PtLen(stPtCandidatePt, endPtCandidatePt)),
					...
					//Elongate Middle Array Point
					k = GeoTools.angleInRadians(midPt, pts[0]);
					var elongateMiddlePt = { x: -1 * length / 10 * Math.cos(k) + midPt.x, y: -1 * length / 10 * Math.sin(k) + midPt.y };
					middleArray = [];
					middleArray = middleArray.concat(elongateMiddlePt, midPt);
					result.addPath(middleArray);
