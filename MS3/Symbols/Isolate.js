...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
  ...
  function (declare, lang, Array,
    ...
    var Isolate = declare([Evented], {
      declaredClass: "MilitarySymbology.Symbols.Isolate",
      SID: "341500",
      symName: "Isolate",
      ...
      init: function (options, marker) {
        ...
        if (options.hasOwnProperty("CTRL_PTS") && options.hasOwnProperty("GEOM")) {
          ...
          drawEssentials = this.createDrawEssentials(lang.clone(options.CTRL_PTS), options.TEETH_SIZE);
      ...
      angleBetweenTwoPointsWithFixedPoint: function (point1X, point1Y, point2X, point2Y, fixedX, fixedY) {
      ...
      createDrawEssentials: function (ctrlPts, teethSize) {
        var drawEssentials = new DrawEssentials();
        ...
        try {
          ...
          if (pts.length === 2) {
            ...
            if (circle.radius > 0) {
              ...
              var cLenLimit;
              var firstPoint = new Point(paths[25][0], paths[25][1], this.map.spatialReference);
              var secondPoint = new Point(paths[35][0], paths[35][1], this.map.spatialReference);
              var baseLineLen = GeoTools._2PtLen(firstPoint, secondPoint);
              cLenLimit = baseLineLen / 5;
              if (cLenLimit > baseLineLen / 3.6) cLenLimit = baseLineLen / 3.6;
              result.addPath(Shapes.createI(cPoint.x, cPoint.y, cLenLimit, this.map.spatialReference));
              //End of Create I


              var p = new Point(this.map.spatialReference);
              var length = GeoTools._2PtLen(endPt, cPoint) / 10;
              var angle = GeoTools.twoPtsAngle(values.backPoint, values.lastPoint);
              ...
              var innerWing = new Point(endPt.x + length * Math.cos(angle), endPt.y + length * Math.sin(angle), this.map.spatialReference);
              var angle = GeoTools.twoPtsAngle(values.backPoint, values.lastPoint);
              ...
              //angle += 0.785398;
              var outerWing = new Point(endPt.x + length * Math.cos(angle), endPt.y + length * Math.sin(angle), this.map.spatialReference);
              result.addPath([innerWing, endPt]);
              result.addPath([outerWing, endPt]);
              ...
              result.addPath(this.createTeeth(result.getPoint(0, 1), result.getPoint(0, 7), centerPt, teethSize));
              result.addPath(this.createTeeth(result.getPoint(0, 9), result.getPoint(0, 15), centerPt, teethSize));
              result.addPath(this.createTeeth(result.getPoint(0, 17), result.getPoint(0, 23), centerPt, teethSize));


              result.addPath(this.createTeeth(result.getPoint(1, 1), result.getPoint(1, 7), centerPt, teethSize));
              result.addPath(this.createTeeth(result.getPoint(1, 9), result.getPoint(1, 15), centerPt, teethSize));
              result.addPath(this.createTeeth(result.getPoint(1, 17), result.getPoint(1, 23), centerPt, teethSize));
      ...
      cleanUp: function () {
        ...
        drawEss = this.createDrawEssentials(lang.clone(this._points), this._teethSize);
      ...
      createTeeth: function (startPt, endPt, centerPt, teethSize) {

        var midPt = GeoTools.getMidPoint(startPt, endPt);
        var angle = GeoTools.twoPtsAngle(centerPt, midPt);
        var midPtTwrdsCntr = new Point(-1 * teethSize * Math.cos(angle) + midPt.x, -1 * teethSize * Math.sin(angle) + midPt.y, this.map.spatialReference);
        return [startPt, midPtTwrdsCntr, endPt];
