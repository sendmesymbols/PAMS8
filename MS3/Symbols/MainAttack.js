/**
 * Class Representing Main Attack.
 * @class
 * @author Abdul Razak
 */
...
define(["dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array",
  ...
  function (declare, lang, Array,
    ...
    var MainAttack = declare([Evented], {
      declaredClass: "MilitarySymbology.Symbols.MainAttack",
      SID: "151403",
      symName: "Main Attk",
      ...
      createDrawEssentials: function (ctrlPts, arrowHeadRatio, tailFactor) {
        ...
        drawEssentials.HEAD_RATIO = this._headPercentage;
        drawEssentials.TAIL_FACTOR = this._tailFactor;
      ...
      createSymbol: function (drawEssentials) {
        try {
          ...
          if (pts.length <= 2) {
            ...
            var values = this.CreateArrowHeadPathEx(p1, lastPoint, p2, len, this._headPercentage, 15);
            ring = ring.concat(values.rings);
            ...
            var midPt = GeoTools.getMidPoint(values.midPtLeft, values.midPtRight);
            var angle = GeoTools.twoPtsAngle(midPt, lastPoint);
            var headBaseLen = len * this._headPercentage / 1.3;

            var newCandiadatePt = { x: midPt.x + headBaseLen * Math.cos(angle), y: midPt.y + headBaseLen * Math.sin(angle) };



            ring = [];
            ring = ring.concat(values.midPtLeft, newCandiadatePt, values.midPtRight);
            result.addRing(ring);

            //GeoTools.displayPoint(this.map, newCandiadatePt);
            //GeoTools.displayPoint(this.map, values.midPtRight);
            //GeoTools.displayPoint(this.map, values.newCandiadatePt);
            ...
            var values = this.CreateArrowHeadPathEx(leftArray[leftArray.length - 1], lastPoint, rightArray[rightArray.length - 1], GeoTools._ptCollectionLen(tempArray, 0), this._headPercentage, 15);
            var headPath = values.rings;
            ...
            //Inner line in Arrow
            ring = [];
            ring = ring.concat(values.midPtLeft, values.newCandiadatePt, values.midPtRight);
            result.addRing(ring);
      ...
      _onMMoveHdler: function (inputPoint) {
        var drawEssentials = this.createDrawEssentials(this._points.concat(inputPoint.mapPoint), this._headPercentage, this._tailFactor);
        this._tGraphic.setGeometry(this.createSymbol(drawEssentials));
      ...
      CreateArrowHeadPathEx: function (pt1, candidatePt, pt2, totalLen, headPercentage, headAngle, straight) {
        ...
        var leftInnerPt = { x: candidatePt.x + result * Math.cos(angle1), y: candidatePt.y + result * Math.sin(angle1) };
        var leftOuterPt = { x: candidatePt.x + headSideLen * Math.cos(angle1 - headAngle / 180 * Math.PI), y: candidatePt.y + headSideLen * Math.sin(angle1 - headAngle / 180 * Math.PI) };
        ...
        var rightInnerPt = { x: candidatePt.x + result * Math.cos(angle2), y: candidatePt.y + result * Math.sin(angle2) };
        var rightOuterPt = { x: candidatePt.x + headSideLen * Math.cos(angle2 + headAngle / 180 * Math.PI), y: candidatePt.y + headSideLen * Math.sin(angle2 + headAngle / 180 * Math.PI) };

        ring.push(rightOuterPt);
        ring.push(rightInnerPt);


        var intersectLineLeft = GeoTools.getMidPoint(leftInnerPt, leftOuterPt);
        var intersectLineRight = GeoTools.getMidPoint(rightInnerPt, rightOuterPt);

        var midPt = GeoTools.getMidPoint(pt1, pt2);
        var angle = GeoTools.twoPtsAngle(midPt, candidatePt);

        var newCandiadatePt = { x: midPt.x + headBaseLen * Math.cos(angle), y: midPt.y + headBaseLen * Math.sin(angle) };


        return { rings: ring, midPtLeft: intersectLineLeft, midPtRight: intersectLineRight, newCandiadatePt: newCandiadatePt };
